...
import { API_REPORT_CONTRACT_NEED_EXTENDED } from '../../../config/urlConfig';
...
export default function ReportContractNeedExtened(props) {
  const { onClose } = props;
  const INITIAL_QUERY = {
  ...
  const getColumns = () => {
    const viewConfig = [];
    viewConfig[0] = { name: 'year', title: 'Năm', checked: true, width: 150 };
    ...
    viewConfig[3] = { name: 'name', title: 'Tên hợp đồng', checked: true, width: 150 };
    viewConfig[4] = { name: 'typeContract', title: 'Loại hợp đồng', checked: true, width: 150 };
    viewConfig[5] = { name: 'contractSigningDate', title: 'Ngày ký', checked: true, width: 150 };
    viewConfig[6] = { name: 'expirationDay', title: 'Ngày kết thúc', checked: true, width: 150 };
  ...
  const handleCloseExcel = () => {
    ...
    if (type === 'PDF') {
      let pdf = tableToPDF(
        'excel-table-contact-need-extended',
        'reportGetContractNeedExtended',
        'Báo cáo danh sách các hợp đồng cần gia hạn',
      );
  ...
  return (
    <>
      <CustomAppBar title={'Danh sách các hợp đồng cần gia hạn'} onGoBack={onClose} disableAdd />
      <Paper style={{ padding: ' 80px 20px 0 20px' }}>
        ...
        <ListPage
          apiUrl={`${API_REPORT_CONTRACT_NEED_EXTENDED}?${serialize(queryFilter)}`}
        ...
        <ExportTable
          ...
          getRows={getRows}
          url={API_REPORT_CONTRACT_NEED_EXTENDED}
          open={exportExcel}
