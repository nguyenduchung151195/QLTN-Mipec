...
class Services extends React.Component {
  ...
  handleSubmitForm = () => {
    if (this.state.isEditting === false) {
      this.props.callBack(`add-${this.props.funcName}`, this.state.dialogData);
    } else {
      this.props.callBack(`update-${this.props.funcName}`, this.state.dialogData);
    }
  ...
  render() {
    ...
    return (
      <div>
            ...
            {selection.length > 0 ? (
              <Fab
                onClick={() => {
                  ...
                  const r = confirm(`Bạn có muốn xóa ${this.props.name} này?`);
                  if (r) {
                    const deleteIds = [];
                    ...
                    this.props.callBack(`delete-${this.props.funcName}`, deleteIds);
                  }
                }}
                size="small"
                color="secondary"
        ...
        >
          <DialogTitle id="customized-dialog-title">{this.state.isEditting ? `Sửa ${this.props.name}` : ` Thêm mới ${this.props.name}`}</DialogTitle>
