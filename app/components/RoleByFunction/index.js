...
const FUNCTION_REORDER = [
  ...
  { code: 'Stock', childCodes: ['Inventory', 'Asset', 'StockExport', 'StockImport', 'StockConfig', 'Allocate'] },
            ...
            {newAllFunctionForAdd.map(row => {
              ...
              if (isChildOpen)
                return (
                  <TableRow key={row.codeModleFunction}>
                    <TableCell>
                      <p onClick={() => this.detailRole(row.codeModleFunction)} style={{ color: '#2196F3', cursor: 'pointer', paddingLeft: '20px' }}>
                        {row.titleFunction}
                      </p>
                    </TableCell>
                    <TableCell>
