...
function AdvanceFilters(props) {
  const { onChange, query: outerQuery , profile} = props;
  const defaultQuery = {
    age: [18, 50],
    ageFilter: false,
    ...
    <>
      <Grid container spacing={8}>
        ...
        {!query.findCustomerWithoutManager ? (
          <Grid item xs={12}>
            <DepartmentAndEmployee department={query.department} employee={query.employee} onChange={handleChangeDepartmentAndEmployee} profile={profile} moduleCode="Customer" />
          </Grid>
        ) : null}
        <Grid item xs={6}>
          <CustomInputField
