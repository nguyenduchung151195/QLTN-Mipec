...
import { AUTOMATION_URL,API_LOGOUT,API_HISTORY_ACTION } from '../../config/urlConfig';
...
class Sidebar extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      openTower: this.activeRoute('/tower'),
  ...
  logout = () => {
    ...
    if (token) {
      ...
      fetchData(API_HISTORY_ACTION, 'POST', dataHistoryAction,'token');
  ...
  render() {
    const { classes, color, logo, image, logoText, routes, bgColor, rtlActive, intl, logoDisplayText} = this.props;
    ...
    const user = (
      <div className={userWrapperClass}>
        <div className={photo}>
          <img style={{ height: 34 }} src={this.props.profile ? this.props.profile.avatar : avatar} className={classes.avatarImg} alt="..." />
        </div>
    ...
    const brand = (
      <div className={logoClasses}>
        ...
        <img  style = {{objectFit: 'contain', height: 35}} src={logo.indexOf('https://g.lifetek.vn:203') > -1 ? logo : logoDefault} alt="logo" className={classes.img} />
        </a>
