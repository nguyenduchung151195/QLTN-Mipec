...
class EditProductInfo extends React.Component {
  ...
  componentWillReceiveProps(props) {
    ...
    if (Object.keys(product).length !== 0 && !this.state.isSubmit && props.product !== this.props.product) {
      ...
      // this.state.avatarURL = product.logo || AvatarImg;
      ...
      this.state.tax = product.tax;
      this.state.environment = product.environment;
      let serials = [];
      if (product.serials) {
  ...
  render() {
    const { classes, suppliersList, tagsList, calculateUnitList } = this.props;
        ...
        <Grid container spacing={16}>
                  ...
                  ? this.state.serialList.map((item, index) => (
                    <Grid key={index} style={{ marginTop: '5px' }}>
                      ...
                      <CustomInputBase
                        ...
                        onChange={e => {
                          if (e.target.value > 0) {
                            this.handleChangeSerial(index, 'value', e);
                          } else {
                            e.preventDefault();
                          }
                        }}
          ...
          <Grid md={5} item>
            ...
            <CustomInputBase
              ...
              error={localMessages && localMessages.environment}
              helperText={localMessages && localMessages.environment}
              required={checkRequired.environment}
              checkedShowForm={checkShowForm.environment}
            />
            <CustomInputBase
            ...
            <FormControl style={{ marginLeft: 15, width: 600, overflowX: 'auto' }}>
              <FileUpload name={this.state.name} id={this.state.productId} size="4px 20px" code="Stock" />
            </FormControl>
          </Grid>
        </Grid>
      </div>
  ...
  handleChangeInput = (e, fieldName) => {
    if (fieldName) {
      ...
      if (fieldName === 'tax' || fieldName === 'environment') {
