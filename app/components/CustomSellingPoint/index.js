...
import { onlyNumberAndSmaller100, noSpaceStartAndEnd, onlyNumber, initialDate, compareDates } from 'config/config';
...
class CustomSellingPoint extends React.Component {
  state = {
    ...
    allowedUsers: [],
    saleStartDay: initialDate(),
    saleEndDay: initialDate(),
  };
  ...
  handleChangeInputBasicInfo = (index, e) => {
    const { departmentList } = this.state;
    departmentList[index][e.target.name] = e.target.value.trim();
    if (e.target.name === 'inventoryChange') {
      departmentList[index].currentQuantity = e.target.value.trim();
    }
    this.setState({ departmentList });
  };
  ...
  render() {
    ...
    const saleDay = compareDates(this.state.saleStartDay, this.state.saleEndDay);
    const saleDayError = saleDay.bool;
    const saleDayHelperText = saleDay.helperText;
    ...
    return (
      <>
        ...
        <div className={classes.expanDiv} style={{ height: 400, overflow: 'auto' }}>
                ...
                <Grid container>
                  ...
                  /> */}
                    ...
                    <TextField
                      ...
                      name="address"
                      margin="normal"
                      onKeyDown={noSpaceStartAndEnd}
                      onPress={noSpaceStartAndEnd}
                    />
                  </Grid>
                  {/* <Grid md={6}> */}
                  ...
                  {/* <FormGroup style={{ marginLeft: 40 }}>
                      ...
                      <div hidden={!place.isSale}>
                        ...
                        <CustomInputBase
                          ...
                          type="date"
                          className={classes.textField}
                          value={this.state.saleStartDay}
                          onChange={e => {
                            this.setState({ saleStartDay: e.target.value });
                            this.handleChangeInputBasicInfo(index, e);
                          }}
                        ...
                        <CustomInputBase
                          ...
                          type="date"
                          className={classes.textField}
                          value={this.state.saleEndDay}
                          error={saleDayError}
                          helperText={saleDayHelperText}
                          onChange={e => {
                            this.props.getValid(!saleDayError);
                            this.setState({ saleEndDay: e.target.value });
                            this.handleChangeInputBasicInfo(index, e);
                          }}
                          name="endDayForSale"
                          margin="normal"
                          ...
                          ? place.taxOptions.map((item, indexTax) => (
                              <div style={{ display: 'flex' }} key={indexTax}>
                                ...
                                <CustomInputBase
                                  ...
                                  className={classes.textField}
                                  onKeyDown={onlyNumberAndSmaller100}
