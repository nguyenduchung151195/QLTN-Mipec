...
const MenuAction = React.memo(({ code, id, itemCurrent, handleClose, anchorEl, addChildTask, extraMenu, getData, miniActive }) => {
  ...
  return (
    <div>
      ...
      <SwipeableDrawer anchor="right" onClose={closeDialogTask} open={openDialogTask} width={miniActive ? window.innerWidth - 80 : window.innerWidth - 260}>
        <AddProjects
          id="add"
          data={{
            source: code,
      ...
      >
        ...
        <TextField
          ...
          select
        >
          {state.templatess && state.templatess.map(form => (
