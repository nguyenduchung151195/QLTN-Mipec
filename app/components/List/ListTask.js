...
class ListPage extends React.PureComponent {
  ...
  componentDidMount() {
    const kanbanList = this.props.kanban
      ? JSON.parse(localStorage.getItem([this.props.status])) &&
        JSON.parse(localStorage.getItem([this.props.status])).find(item => item.code === this.props.kanban).data
      : [];
    ...
    fetch(URL, {
      ...
      .then(respon => {
        ...
        if (respon.status == 200) {
          ...
          this.setState({ loading: false, open: true, variant: 'error', message: this.state.message || 'Xóa không thành công' });
        }
      })

      .catch(e => {
        console.log('33333', e);
        this.setState({ loading: false, open: true, variant: 'error', message: e.message || 'Không có phản hồi, kiểm tra lại kết nối' });
      });
  };
  ...
  render() {
    ...
    const roleCode =
      ...
      this.props.dashboardPage.role.roles.find(item => item.codeModleFunction === this.props.code);
    ...
    const extra = roleDashboard.find(elm => elm.code === 'EXTRA') ? roleDashboard.find(elm => elm.code === 'EXTRA').data : null;

    const hideTask = extra && extra.find(elm => elm.name === 'hide').data;
    const protectedTask = extra && extra.find(elm => elm.name === 'protected').data;
    const publicTask = extra && extra.find(elm => elm.name === 'public').data;
    const openTask = extra && extra.find(elm => elm.name === 'open').data;
    const derpartment =
      this.props.dashboardPage.role &&
      this.props.dashboardPage.role.roles &&
      this.props.dashboardPage.roleTask.roles.find(elm => elm.code === 'DERPARTMENT').data;
    const organizationUnit = this.props.dashboardPage.profile.organizationUnit
      ? this.props.dashboardPage.profile.organizationUnit.organizationUnitId
      : '';

    const derpartmentRole = organizationUnit ? derpartment && derpartment.find(elm => elm.name === organizationUnit) : '';
    const bussinesRole = this.props.dashboardPage.roleTask.roles.find(elm => elm.code === 'BUSSINES').data;
    return (
      <GridContainer style={{ backgroundColor: '#ffffff', paddingTop: '20px' }}>
        ...
        <GridItem md={6} sm={6} style={{ alignItems: 'center', display: 'flex', justifyContent: 'flex-end' }}>
          ...
          // )
          null}
          {code && this.state.hasPermissionViewConfig ? (
    ...
    return (
      <div>
        <Menu open={Boolean(anchorEl)} onClose={handleClose} anchorEl={anchorEl}>
          ...
          {/* <MenuItem onClick={makeCreateTaskRequest}>Đề xuất CV</MenuItem> */}
          ...
          {disableEmail ? '' : <MenuItem onClick={handleEmailDialog}>Gửi mail</MenuItem>}
          {disableCall ? '' : <MenuItem>Gọi điện</MenuItem>}
          {disableSms ? '' : <MenuItem onClick={handleSMSDialog}>SMS</MenuItem>}
        ...
        >
          ...
          />
          <TextField label="Tên quy trình" name="name" onChange={e => handleChangeApproved(e, 'subCode')} value="Task" />
