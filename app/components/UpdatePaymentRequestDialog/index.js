...
class UpdatePaymentRequestDialog extends React.Component {
  ...
  componentWillReceiveProps(props) {
    ...
    if (props.open !== this.props.open && props.currentStage !== null) {
      ...
      if (props.currentStage.currency === '%' && (Number(amount) / Number(props.debtMoney)) * 100 > 0.1) {
        this.state.currentStage.amount = (Number(amount) / Number(props.debtMoney)) * 100;
      }
    }
    // }
    if (this.props.taskChoose !== null) {
    ...
    if (props.open !== this.props.open && !isNaN(props.debtMoney) && props.currentStage.amount <= 0) {
      const { currentStage } = this.state;
      currentStage.amount = Number(props.debtMoney);
  ...
  handleChangePrice = e => {
    ...
    if (e.target.name !== 'amount') {
      ...
      if (Number(e.target.value) > Number(this.props.debtMoney)) {
        ...
        currentStage.amount = e.target.value;
        this.setState({ currentStage });
      }
    }
  };
  ...
  handleSave = () => {
    ...
    if (currentStage.currency === '%') {
      ...
      currentStage.amount = (Number(this.props.debtMoney) * Number(amount)) / 100;
    }
    // if (currentStage.amount >= this.props.debtMoney) {
