...
import { UPLOAD_APP_URL, API_ASSET, API_STOCK_IMPORT, API_STOCK_EXPORT } from '../../config/urlConfig';
...
class ApproveDialog extends React.Component {
  ...
  componentDidMount() {
    ...
    const isFinished = data && data.dataInfo && data.dataInfo.isFinished;
    const code = data && data.dataInfo && data.dataInfo.code;
    const type = data && data.dataInfo && data.dataInfo.type;
    // const state = data && data.dataInfo && data.dataInfo.state;
    const approveIndex=data && data.approveIndex;
    const filter = {
      // limit: 10,
      // skip: 0,
      // sort: '-updatedAt',
    ...
    if ((type === 3 || type === 7) && isFinished === true && (Number(approveIndex)===0)) {
      fetch(`${API_STOCK_EXPORT}?${serialize(filter)}`, {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` },
      }).then(res => res.json())
        .then(data => this.setState({
          stockExport: data.data && data.data[0] ? data.data[0] : {}
        }))
    }
  }
  ...
  render() {
    ...
    const dataInfo = this.props.dialogData && this.props.dialogData.dataInfo ? this.props.dialogData.dataInfo: {};
    const isFinished = dataInfo.isFinished;
              ...
              <IntegratedPaging /> */}
                ...
                {this.checkCurrentUserTurn(currentUser, dialogData) ? (
                  ...
                  >
                    ...
                    {(!isFinished || (isFinished && this.state.stockExport && (Number(this.state.stockExport.state) === 4|| Number(this.state.stockExport.state) === 1))) && (
                      <>
                        <Fab
