...
function DepartmentAndEmployee(props) {
  ...
  // const [innerEmployee, setInnerEmployee]
  ...
  // console.log(1, roleDepartment.map(it => it.view == true))
  return (
    <>
        ...
        {Array.isArray(roleDepartment) && roleDepartment.find(it => it.view == true) ? (
          !disableEmployee ? (
            <Grid style={{ justifyContent: 'flex-end', display: 'flex' }} md={12}>
              <Grid item md={isReport === true ? 12 : 6}>
                <CustomInputBase
                  isReport={isReport}
                  value={localData.department}
                  onChange={handleDepartment}
                  label={isProject ? 'Dự án' : labelDepartment || intl.formatMessage(messages.department)}
                  select
                >
              ...
              {isReport === false && (
                <Grid item md={typeWidth ? 10 : 6} className={isReport && 'reportEmployeeStyle'}>
                  <AsyncAutocomplete
                    ...
                    filters={['name', 'code']}
                    customOptionLabel={option => `${option.name}${option.code ? ` - ${option.code}` : ''}`}
