...
class AutomationDialog extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState(state => {
      let columns = [];
      let others = [];
      state.localViewConfig && state.localViewConfig.filter(x => x.code == this.props.viewConfig.code).map(item => {
  ...
  changeField(value, index) {
    ...
    this.setState(state => {
      valueAsync.map(x => { console.log(x) })
      state.dialogData.actions[index].sendTo = valueAsync.map(item => ({
        ref: item.type && this.genMessageForRelationiField(item.type) || null, field: item.field, name: item.name, _id: item._id
      }))
      return state;
    })
  }
  render() {
    ...
    const newListFields = this.props.nameViewConfig === false ? this.state.fields.filter(x => x.type.includes('ObjectId')).map((item, index) => ({
      ...item, _id: `${item.name}-${index}`, name: item.title, field: item.name
    })) :
    ...
    const columnListFieldApplys = calendarViewConfig && loadash.has(calendarViewConfig, 'listDisplay.type.fields.type') ? calendarViewConfig.listDisplay.type.fields.type.columns : [];
    const othersListFieldApplys = calendarViewConfig && loadash.has(calendarViewConfig, 'listDisplay.type.fields.type') ? calendarViewConfig.listDisplay.type.fields.type.columns : [];
    ...
    return (
      <div>
        ...
        >
                        ...
                        <Grid item sm={4}>
                          <TextValidator
                            ...
                            onChange={event => {
                              ...
                              dialogData.conditions[index].valueType = listField[listField.findIndex(d => d.name === event.target.value)].type;
                              dialogData.conditions[index].configType = listField[listField.findIndex(d => d.name === event.target.value)].configType ? listField[listField.findIndex(d => d.name === event.target.value)].configType : null;
                              dialogData.conditions[index].configCode = listField[listField.findIndex(d => d.name === event.target.value)].configCode ? listField[listField.findIndex(d => d.name === event.target.value)].configCode : null;
                              dialogData.conditions[index].options = listField[listField.findIndex(d => d.name === event.target.value)].menuItem ? listField[listField.findIndex(d => d.name === event.target.value)].menuItem : null;
                              this.setState({ dialogData });
                            }}
                            select
                            ...
                            (dialogData.conditions[index].valueType === 'Date' ? (
                              ...
                              <CustomInputField
                                ...
                                onChange={event => {
                                  ...
                                  const { dialogData } = this.state;
                                  dialogData.conditions[index].valueInfor = dialogData.conditions[index].valueType.includes('Source') || dialogData.conditions[index].valueType.includes('MenuItem') || dialogData.conditions[index].valueType==='Number' ? event.target.value : event;
                                  dialogData.conditions[index].value = dialogData.conditions[index].valueType.includes('Source') || dialogData.conditions[index].valueType.includes('MenuItem') ? event.target.value.type : event._id;
                                  this.setState({ dialogData });
                                }}
                    ...
                    <Card>
                      <CardContent>
                        <Grid container alignItems="center">
                          ...
                          <Grid item sm={6}><h5>Nơi nhận</h5></Grid>
                          <Grid item sm={12}>
                            <Grid>
                              <Autocomplete
          ...
          <DialogActions>
            {isEditting && dialogData.originData && <Button
              onClick={() => {
                this.setState({ dialogData: {
