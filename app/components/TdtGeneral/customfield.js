    ...
    case 'Money': {
      ...
      fieldCode = (
        <Grid container justify="center" alignItems="center" spacing={8}>
          ...
          <Grid item xs={12}>
            {
              localMessages["value.amount"] ? <Typography color="error">
                {localMessages["value.amount"]}
              </Typography> : localMessages["value.currencyUnit"] ? <Typography color="error">{localMessages["value.currencyUnit"]}</Typography> : ''
            }
          </Grid>
        </Grid>
      );
        ...
        row.name !== 'contactCenter' && row.name !== 'businessOpportunities' ? (
          <div className="my-2">
            <Typography color={localMessages['customer.name'] ? "secondary" : ""}>
              {row.checkedShowForm ? getLabelName(`${row.name}.name`, `${isTrading ? 'ExchangingAgreement' : 'BusinessOpportunities'}`) : ''} {row.checkedRequireForm ? '*' : ""}
            </Typography>
            {
              row.checkedShowForm ?
                generalData[`${row.name}.name`] ? (
                  ...
                  <Avatar alt="Remy Sharp">
                    ...
                    <React.Fragment>
                      ...
                      {
                        localMessages["customer.name"] ? <Typography color="error">
                          {localMessages["customer.name"] ? localMessages["customer.name"] : ''}
                        </Typography> : ''
                      }
                    </React.Fragment>
                  )
    ...
    case 'People Array': {
      ...
      fieldCode =
        row.name !== 'contactCenter' ? (
          row.checkedShowForm ? <div className="my-2">
            <p
              // className="font-weight-bold mb-1" 
              style={{ color: localMessages[`${row.name}`] ? 'red' : '' }}>{getLabelName(`${row.name}`, `${isTrading ? 'ExchangingAgreement' : 'BusinessOpportunities'}`)}
            </p>
            <AsyncSelect
