...
function PlanDemo(props) {
  const { apiUrl, addItem, reload, code, filterItem, profile, id, showTemplate, callback } = props;
  const categoryTask = JSON.parse(localStorage.getItem('taskStatus')).find(item => item.code === 'TASKTYPE').data;
  ...
  useEffect(
    () => {
      if (
        ...
        setAllowAdd((roleModule.find(elm => elm && elm.name === 'POST') || { allow: false }).allow);
        setAllowEdit((roleModule.find(elm => elm && elm.name === 'PUT') || { allow: false }).allow);
      }
    },
    [props.dashboardPage],
  );
  ...
  useEffect(
    () => {
      if (showTemplate) {
        ...
        if (template) {
          newProjectFilter.template = template && template._id;
        }
  ...
  async function reloadData(load = true) {
    ...
    try {
      ...
      const newColumns = JSON.parse(localStorage.getItem('taskStatus'))
      ...
      responses.forEach((item, index) => {
        newDataCount[newColumns[index].type] = item.count;
  ...
  async function loadDataProject(load = true) {
    ...
    if (id) {
      ...
      filter.template = template && template._id;
    }
    if (filter) {
  ...
  function updateKanbanStatus(item) {
    const updateData = url => {
      fetchData(`${url}/${item && item._id}`, 'PUT', item).then(res => {
        if (res.status === 1 || res.success) setSnackbar({ variant: 'success', message: 'Cập nhật thành công', open: true });
        else {
  ...
  function updateProgress(item) {
    const updateData = url => {
      ...
      data.taskId = item && item._id;
      const projectId = item && item.projectId ? item.projectId._id : item._id;
    ...
    <Paper>
      <div className="kanban-main">
        ...
        <Dialog dialogAction={false} onClose={closeDialog} open={open}>
          <TaskDialog setCoverTask={setCoverTask} profile={profile} taskId={taskItem && taskItem._id} filterItem={innerFilterItem} data={taskItem} />
        </Dialog>

        <Grid container spacing={8} alignItems="center">
          <Grid item style={{ width: 'calc(60%/3)' }}>
            <TextField
          ...
          {document.documentElement.clientWidth <= 1285 ? (
            ...
            <Grid item style={{ marginLeft: 8, width: 'calc((60%/3)*2)' }}>
          ...
          <Grid item style={{ width: 'calc(60%/3)' }}>
            <CustomInputBase onChange={handleFilter} select value={filters} label="Thời gian" variant="outlined" fullWidth margin="dense">
              {third.map(option => (
          ...
          <Grid item>
            <Fab onClick={() => setDialogPlanFilter(true)} color="primary" style={{ marginLeft: 15, width: 40, height: 40, minWidth: 40 }}>
            ...
            <Grid item style={{ width: '100%' }}>
              <TextField value={(category !== 0 ? category : agreeCategory) || 0} select fullWidth onChange={handleCategory} label="Loại công việc">
                ...
                {categoryTask.map((item, index) => (
                  <MenuItem value={index + 1}>{item ? item.name : ''}</MenuItem>
                ))}
              </TextField>
            </Grid>
...
const Column = React.memo(props => {
  const { color, name, data, id, addItem, openDialog, code, reloadData, count, noLoadmore, isReset, allowAdd, allowEdit, callback } = props;
  const [loading, setLoading] = React.useState(false);
...
const Item = React.memo(({ data, index, openDialog, reloadData, allowEdit, callback }) => {
  const [open, setOpen] = React.useState(false);
  ...
  return (
    <React.Fragment>
      {data && (
        <Draggable index={index} draggableId={data && data._id}>
          {(provided, snapshot) => (
            <div
            ...
            >
              <div className="tasks-kanban-item">
                ...
                {data && data.projectId ? <p>Dự án: {data && data.projectId ? data.projectId.name : ''}</p> : null}
                <p>Tiến độ: {(data && data.progress * 1).toFixed(2)}%</p>
                <p>
      ...
      <Dialog
        ...
        dialogAction={false}
      >
        <ViewContent code="Task" id={data && data._id} />
      </Dialog>
      <SwipeableDrawer onClose={closeDialog} open={open} width={window.innerWidth - 260}>
        <AddTask id={data && data._id} reloadData={reloadData} onClose={closeDialog} callback={callBack} />
      </SwipeableDrawer>
    </React.Fragment>
  );
});
    ...
    case 3:
      return (
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end' }}>
          {joins.map(i => (
            <Tooltip className="kanban-avatar" placement="top-start" key={i && i._id} title={i && i.name}>
              <Avatar alt="Nguyễn văn A" src={i ? i.avatar : lang} />
            </Tooltip>
          ))}
          {add ? (
    ...
    default:
      return (
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', marginLeft: 7 }}>
          {joins && (
            <>
              <Tooltip className="kanban-avatar kanban-avatar-firt" placement="top-start" title={joins[0] && joins[0].name}>
                <Avatar alt={joins[0] && joins[0].name} src={joins[0] ? joins[0].avatar : lang} />
              </Tooltip>
              <Tooltip className="kanban-avatar" placement="top-start" title={joins[1] && joins[1].name}>
                <Avatar alt={joins[1] && joins[1].name} src={joins[1] ? joins[1].avatar : lang} />
              </Tooltip>
              <Tooltip className="kanban-avatar" placement="top-start" title={joins[2] && joins[2].name}>
                <Avatar alt={joins[2] && joins[2].name} src={joins[2] ? joins[2].avatar : lang} />
              </Tooltip>
            </>
          )}
          ...
          <Menu onClose={() => setAnchorEl(null)} open={Boolean(anchorEl)} anchorEl={anchorEl}>
            {length > 0 &&
              joins.map(i => (
                <MenuItem key={i && i._id}>
                  <Tooltip className="kanban-avatar" placement="top-start" title={joins[1] ? joins[1].name : ''}>
                    <Avatar alt={i && i.name} src={i && i.avatar} />
                  </Tooltip>
                  {i ? i.name : ''}
                </MenuItem>
              ))}
...
function TaskDialog(props) {
  ...
  useEffect(() => {
    ...
    const projectId = data && data.projectId ? data.projectId._id : data._id;
  ...
  function changeFile(e) {
    ...
    setCurrentFile({ fileName: uploadData && uploadData.name, size: uploadData.size, originFile: uploadData.type, type, taskId, data: uploadData });
    if (type === 'image') {
      ...
      <div className="task-dialog-content-section">
        <div className="task-dialog-content-row">
          <div className="task-section-header">
            <Assignment />
            <b className="task-dialog-name">{state && state.name}</b>
          </div>
        </div>
        ...
        <div className="task-dialog-user-label dialog-ct">
          ...
          <div>
            ...
            <span style={{ background: taskPriotyColor[state.priority - 1].color }} className="task-dialog-priority">
              {taskPriotyColor && taskPriotyColor[state.priority - 1] && taskPriotyColor[state.priority - 1].name}
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="task-dialog-user">
      ...
      <Menu anchorEl={anchorEl} onClose={() => setAnchorEl(null)} open={Boolean(anchorEl)}>
        {props.joins.map(i => (
          <MenuItem>
            <Checkbox color="primary" /> <Avatar style={{ width: 25, height: 25, margin: 5 }} src={i && i.avatar} /> {i ? i.name : ''}
          </MenuItem>
        ))}
      </Menu>
    </div>
