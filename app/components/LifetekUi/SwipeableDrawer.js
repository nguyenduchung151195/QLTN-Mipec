...
import './drawer.css';
// sua o day
const getWidth = () => window.innerWidth - 260;
const LtSwipeableDrawer = props => {
  ...
  return (
    <Drawer anchor="right" onClose={props.onClose} open={props.open} onOpen={props.onOpen}>
      {/* 1.BUTTON - ĐÓNG */}
      {props.disableClose ? null : props.number ? (
        <IconButton style={{ position: 'fixed', top: '0px', right: '10px', zIndex: props.number }} onClick={props.onClose}>
          <Tooltip title="Đóng">
      ...
      {props.disableClose ? (
        <div style={{ width, paddingTop: '0px' }}>{props.children}</div>
      ) : (
        <div style={{ width, paddingTop: '0px' }}>{props.children}</div>
      )}
