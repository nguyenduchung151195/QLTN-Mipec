...
function Comments({ id, code, limit = 10, newComment }) {
  ...
  const handleUploadFile = async e => {
    ...
    const newImageList = [...imageList, { file: 'Đang tải lên...', fileName: file.name, isImage: isImage }];
    ...
    console.log('newImageList2', newImageList);
  };
    ...
    <div className="dialog-ct">
      ...
      {profile._id && (
        <div className="reply-commnent">
          <Avatar style={{ width: 30, height: 30 }} alt={profile.name} src={profile.avatar} />
          <TextField
            multiline
            style={{ marginLeft: 10 }}
            fullWidth
...
function Comment({ item, reply, profile, code, id, limit, deleteComment }) {
  ...
  function sendComment(data) {
    try {
      ...
      fetchData(`${API_COMMENT}`, 'POST', data).then(result => {
        const it = [{ ...result, user: { name: profile.name, avatar: profile.avatar, _id: profile._id }, time: convertDateFb(result.createdAt) }];
  ...
  return (
    <div style={{ borderRadius: '10px', margin: '15px 0px' }}>
            ...
            <Grid container spacing={3}>
              <Grid item xs={2}>
                <Avatar
                  ...
                  src={item.user ? item.user.avatar : 'https://techtalk.vn/wp-content/uploads/2019/03/how-to-become-blockchain-developer-696x433.jpg'}
                />
                <p style={{ whiteSpace: 'pre-wrap' }}>
                  <span style={{ cursor: 'pointer', fontWeight: 'bold', marginRight: 5 }}>{item.user ? item.user.name : 'Admin'}</span>
                  <br />
                  {item.content}
                  {item.image &&
                    item.image.length > 0 && (
      ...
      <div style={{ marginLeft: 30 }}>
        <div>
          <span onClick={likeComment} style={{ marginRight: 6, cursor: 'pointer', color: '#385898' }}>
            Thích
          </span>
            ...
            <div style={{ borderRadius: '10px', margin: '15px 10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', marginLeft: 20 }}>
                <Avatar
                  ...
                  src={i.user ? i.user.avatar : 'https://techtalk.vn/wp-content/uploads/2019/03/how-to-become-blockchain-developer-696x433.jpg'}
                />
                <p>
              ...
              <div style={{ marginLeft: 30 }}>
                <div>
                  <span style={{ marginRight: 6, cursor: 'pointer', color: '#385898' }}>Thích</span>
                  <button onClick={() => reply(item._id)} type="button" style={{ cursor: 'pointer', color: '#385898', marginRight: 6 }}>
                    Trả lời
                  </button>
                  <span>{i.time}</span>
      ...
      {item.reply ? (
        <InputComment
          ...
          avatar={profile.avatar ? profile.avatar : 'https://techtalk.vn/wp-content/uploads/2019/03/how-to-become-blockchain-developer-696x433.jpg'}
        />
      ) : null}
    </div>
  );
...
function InputComment(props) {
  ...
  return (
    <div className="input-container">
      <Avatar style={{ width: 26, height: 26 }} src={props.avatar} />
