...
export default function KanbanStep({ kanbanStatus, code, typeStatus, handleStepper, Id, apiUrl, disableClick }) {
      ...
      <div>
        <Stepper style={{ background: 'transparent' }} activeStep={idx}>
          {currentStatus.map(item => (
            <Step onClick={() => disableClick && item.type === '3' ? null : handleStep(item)} key={item.type}>
