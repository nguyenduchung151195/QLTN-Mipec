...
import { clientId, logNames } from '../../variable';
import { API_USERS, API_TEMPLATE, API_APPROVE_GROUPS, API_ROLE_GROUP } from '../../config/urlConfig';
...
class Treeview extends React.Component {
  ...
  render() {
    const { treeData, isApproved, template, listTemplates, isObligatory, dataUser, dataRoleTg } = this.state;
    ...
    return (
      <Grid container>
        ...
        <Grid style={{ display: 'flex', justifyContent: 'right' }} item md={6}>
          <Button style={{ marginRight: 15 }} color="primary" variant="outlined" onClick={() => this.toggleNodeExpansion(true)}>
            Mở rộng tất cả
          </Button>
          ...
          <Autocomplete
            ...
            onChange={value => {
              ...
              if (length && value[length - 1].roles) {
                ...
                this.setState({ join: dataJoin });
              } else {
                this.setState({ join: value ? value : [] });
              }
            }}
