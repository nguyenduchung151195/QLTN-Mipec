...
import { Grid, withStyles, FormControlLabel, FormHelperText, Checkbox, FormControl, MenuItem, Typography, Fab, Button } from '@material-ui/core';
import { withSnackbar } from 'notistack';
import { Link } from 'react-router-dom';
import { compose } from 'redux';
import { CameraAlt, Delete } from '@material-ui/icons';
import { FileUpload, AsyncAutocomplete, TextField } from '../LifetekUi';
...
import { onlyNumber, onlyTextAndNumber } from 'config/config';
...
class ProductInfo extends React.Component {
  ...
  render() {
    const { classes, suppliersList, tagsList, calculateUnitList, intl, messages, disabled } = this.props;
      ...
      <div>
        <Grid container spacing={16}>
          <Grid md={2} item>
            ...
            >
              <img src={this.state.avatarURL || AvatarImg} alt="Ảnh sản phẩm" className={classes.avatar} />
              <input
                accept="image/*"
                  ...
                  ? this.state.serialList.map((item, index) => (
                    <Grid key={index} style={{ marginTop: '5px' }}>
                      <TextField
                        ...
                        variant="outlined"
                        onChange={e => {
                          this.handleChangeSerial(index, 'serialName', e);
                        }}
                        onKeyDown={onlyTextAndNumber}
                        onPress={onlyTextAndNumber}
                        className={classes.textField}
                      ...
                      <CustomInputBase
                        ...
                        className={classes.textField}
                        value={item.value}
                        onChange={e => (e.target.value >= 0 ? this.handleChangeSerial(index, 'value', e) : null)}
                        style={{ width: '40%', marginTop: 0 }}
                        disabled={disabled}
            ...
            <FormControl className={classes.textField}>
              ...
              {!disabled && (
                <Link
                  to="/Stock/config"
                  onClick={() => {
                    localStorage.setItem('stockConfig', 4);
                  }}
                  style={{ display: 'inline-block', textAlign: 'right' }}
                >
                  {`${intl.formatMessage(messages.quanli || { id: 'quanli', defaultMessage: 'quanli' })} mã đơn vị`}
                </Link>
              )}
            </FormControl>
            <FormControl className={classes.textField}>
              ...
              {!disabled && (
                <Link
                  to="/Stock/config"
                  onClick={() => {
                    localStorage.setItem('stockConfig', 0);
                  }}
                  style={{ display: 'inline', textAlign: 'right' }}
                >
                  {`${intl.formatMessage(messages.quanli || { id: 'quanli', defaultMessage: 'quanli' })} ${true ? 'danh mục sản phẩm' : getLabelName('catalog.name', 'Stock')
                    }`}
                </Link>
              )}
            </FormControl>
          ...
          <Grid md={5} item>
            ...
            <CustomInputBase
              ...
              onChange={e => {
                this.handleChangeInput(e, 'tags');
                this.props.parentCallback(e.target);
              }}
              className={classes.textField}
              disabled={disabled}
            ...
            {!disabled && (
              <Link
                to="/Stock/config"
                onClick={() => {
                  localStorage.setItem('stockConfig', 1);
                }}
                style={{ display: 'block', textAlign: 'right', marginRight: '30px' }}
              >
                {`${intl.formatMessage(messages.quanli || { id: 'quanli', defaultMessage: 'quanli' })} loại sản phẩm`}
              </Link>
            )}
              ...
              <CustomInputBase
                ...
                onChange={e => {
                  ...
                  if (!regex.test(e.target.value)) {
                    if (e.target.value === '') {
                      this.setState({ sizeError: false, sizeHelper: '' });
                    } else {
                      this.setState({ sizeError: true, sizeHelper: 'Phải nhập cả chữ và số.' });
                    }
                  } else {
                    this.setState({ sizeError: false, sizeHelper: '' });
                  }
                  this.handleChangeInput(e, 'size');
                }}
                onKeyDown={onlyTextAndNumber}
                onPress={onlyTextAndNumber}
                disabled={disabled}
                error={this.state.sizeError}
                helperText={this.state.sizeHelper}
            ...
            <FormControl className={classes.textField}>
              ...
              {!disabled && (
                ...
                <Link
                  to="/Stock/config"
                  onClick={() => {
                    localStorage.setItem('stockConfig', 6);
                  }}
                  style={{ display: 'block', textAlign: 'right', marginRight: '30px' }}
                >
                  {`${intl.formatMessage(messages.quanli || { id: 'quanli', defaultMessage: 'quanli' })} ${true ? 'nhà cung cấp' : getLabelName('supplier.name', 'Stock')
                    }`}
                </Link>
              )}
            </FormControl>
            <CustomInputBase
              ...
              onChange={e => this.handleChangeInput(e, 'tax')}
              type="number"
              disabled={disabled}
              onKeyDown={onlyNumber}
            />
            <CustomInputBase
              ...
              onChange={e => this.handleChangeInput(e, 'environment')}
              type="number"
              disabled={disabled}
              onKeyDown={onlyNumber}
            />
  ...
  getData = () => {
    const { name, code, calculateUnit, productCate, supplier } = this.state;
    // const rex = /^[A-Za-z0-9]+$/;
    // if (
    //   name.trim() === '' ||
