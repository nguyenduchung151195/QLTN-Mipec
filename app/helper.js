...
import { API_TEMPLATE, API_COMMON, API_CUSTOMERS, API_TASK_PROJECT, API_USERS, API_STOCK, API_SALE, API_COMMON_FORM } from './config/urlConfig';
import { EXPORT_FILE_NAME } from './contants';
import moment from 'moment';
import { getCurrentUrl } from './utils/common';
import XLSX from 'xlsx';

export async function convertTemplate({ content, data, code }) {
  let newData = dot.dot(data);
  ...
  function getName(codeName, refName) {
    ...
    if (list) {
      list &&
      list.listDisplay &&
      list.listDisplay.type &&
      list.listDisplay.type.fields &&
      list.listDisplay.type.fields.type &&
      list.listDisplay.type.fields.type.columns
      ...
      list && list.listDisplay && list.listDisplay.type && list.listDisplay.type.fields && list.listDisplay.type.fields.type.others
  ...
  const viewFind =
    ...
    viewConfig.find(item => item.code === code).listDisplay &&
    viewConfig.find(item => item.code === code).listDisplay.type &&
    viewConfig.find(item => item.code === code).listDisplay.type.fields &&
    viewConfig.find(item => item.code === code).listDisplay.type.fields.type
      ? viewConfig.find(item => item.code === code).listDisplay.type.fields.type
      : null;
  ...
  codes.forEach(item => {
    if (item.type.includes('ObjectId')) {
      ...
      content = content.replaceAll(`{${item.name}}`, data[item.name]);
    }
  });
  ...
  async function replaceExtra() {
    for (const item of extra) {
      ...
      const rep = await item.function(newData, item.name);
      content = await content.replace(regex, rep);
    }
  }
  if (extra.length === 0) {
  ...
  // else {
  //   content = findCal({ content, data: newData });
  //   const newResult = codes;
  //   console.log(newResult, 'replace', content);
  //   newResult.forEach(item => {
  //     const replace = `{${item.name}}`;
  //     const regex = new RegExp(replace, 'gs');
  //     const rep = newData[item.name] ? newData[item.name] : '';
...
export function tableToPDF(id, code, fileName = 'PDF', moduleCode, titleHeader = '') {
  ...
  html += `<h2 style="text-align : center ;text-transform: uppercase ">${titleHeader}</h2>`;
  html += `<title>${fileName}</title>`;
  html += style;
  html += '</head>';
  html += '<body>';
  if (moduleCode !== 'report-stock') {
    html += `<h2 style="margin-left: 250px">${fileName}</h2>`;
  }
  html += `<table>`;
  html += sTable;
  html += `</table>`;
...
export function exportTableToExcel(tableID, filename = '', titleHeader = '') {
  var downloadLink;
  var dataType = 'application/vnd.ms-excel';
  var tableSelect = document.getElementById(tableID);
  var tableHTML = tableSelect.outerHTML.replace(/ /g, '%20');
  titleHeader = titleHeader.replace(/ /g, '%20');
  // Specify file name
  ...
  if (navigator.msSaveOrOpenBlob) {
    ...
    downloadLink.href = 'data:' + dataType + `,<h3>${titleHeader}</h3> ` + tableHTML;
...
export function exportTableToExcelXSLX(tableID, filename, dl) {
  var elt = document.getElementById(tableID);
  var wb = XLSX.utils.table_to_book(elt, { sheet: 'sheet1' });
  return dl ? XLSX.write(wb, { bookType: type, bookSST: true, type: 'base64' }) : XLSX.writeFile(wb, `${filename}.` + 'xlsx');
}
...
export function printTemplte(templateId, dataId, moduleCode, isClone = false) {
...
export function printTemplteExcel(templateId, dataId, moduleCode, isClone = false) {
  const templateData = fetchData(`${API_TEMPLATE}/${templateId}`);
  const dataItem = fetchData(`${API_COMMON}/${moduleCode}/${dataId}`);
  Promise.all([templateData, dataItem]).then(result => {
...
export function printAllTemplatePDF(templateId, moduleCode, isClone = false, selected = []) {
  const templateData = fetchData(`${API_TEMPLATE}/${templateId}`);
  let newModuleCode = `${moduleCode.toLowerCase()}s`;
  if (moduleCode === 'Documentary') {
    newModuleCode = `${moduleCode.toLowerCase()}`;
  } else if (moduleCode === 'StockImport') {
    newModuleCode = `stock-import`;
  } else if (moduleCode === 'StockExport') {
    newModuleCode = 'stock-export';
  } else if (moduleCode === 'Asset') {
    newModuleCode = 'asset';
  } else if (moduleCode === 'Contract') {
    newModuleCode = 'contract';
  } else if (moduleCode === 'Stock') {
    newModuleCode = 'inventory';
  }
  const dataItem = fetchData(`${API_COMMON_FORM}/${newModuleCode}`);
  ...
  Promise.all([templateData, dataItem]).then(result => {
    if (selected.length === 0) {
      ...
      const { data = [] } = item2;
      selected.map(select => {
        feeSelected.push(data[select]);
      });
      PrintElemAll({ content: result[0].content, data: feeSelected, code: moduleCode, isClone });
    }
  });
}

export function printAllTemplateExcel(templateId, moduleCode, isClone = false, selected = []) {
  console.log('printy');
  const templateData = fetchData(`${API_TEMPLATE}/${templateId}`);
  let newModuleCode = `${moduleCode.toLowerCase()}s`;
  if (moduleCode === 'Documentary') {
...
const getData = async (content, data, code) => {
  return Promise.all(data.map(item => convertTemplate({ content, data: item, code })));
};
export async function PrintElemAll({ content, data, code, isClone }) {
  // check11
  if (isClone) data = await apiCloneModule(data, code);
  let elements = '';
  if (data && Array.isArray(data)) {
    elements = await getData(content, data, code);
  }
  ...
  await mywindow.document.write(`${elements}${style}`);
...
function getNameByCode(code) {
  switch (code.toLowerCase()) {
    case 'fee':
      return 'Danh sách thông báo phí';
    case 'stock':
      return 'Danh sách kho';
    case 'delivery':
      return 'Danh sách công văn';
    case 'customer':
...
export async function PrintElemAllExcel({ content, data, code, isClone }) {
  if (isClone) data = await apiCloneModule(data, code);
  let elements = '';
  if (data && Array.isArray(data)) {
    elements = await getData(content, data, code);
  ...
  let tdStyles = document.querySelectorAll(`table tr td`);
  tdStyles.forEach(td => {
  ...
  let tableName = getNameByCode(code);
  // tableToExcelAll(tableSelect, tableName);
  ...
  if (navigator.msSaveOrOpenBlob) {
    ...
    navigator.msSaveOrOpenBlob(blob, tableName);
  } else {
    // Create a link to the file
    downloadLink.href = 'data:application/vnd.ms-excel,' + tableHTML;

    // Setting the file name
    downloadLink.download = tableName;
...
export async function PrintElem({ content, data, code, isClone }) {
  if (isClone) data = await apiCloneModule(data, code);
  data.payMethod = data.payMethod === '1' ? 'Chuyển khoản' : 'Tiền mặt';
  ...
  await mywindow.document.write(`${element}${style}`);
  mywindow.document.close(); // necessary for IE >= 10
  mywindow.focus(); // necessary for IE >= 10*/
  mywindow.print();
  // mywindow.close();
...
export function convertDateFb(d1) {
  const end = moment();
  const date = moment(d1).utc();
  let day = end.diff(date, 'days');
  let hours = end.diff(date, 'hours');
  let minutes = end.diff(date, 'minutes');
  if (day === 0) {
    if (hours === 0) {
...
export async function printGroupTask(data) {
  ...
  <td ${isArray(item.child) ? `rowspan=${item.child.length + 1}` : ''}><p style="margin-bottom:0in;margin-bottom:.0001pt;
    text-align:center;line-height:normal" class="MsoNormal"><strong><span style="font-family: 'Times New Roman', Times,serif;">${
      item.name
    }</span></strong></p>
  </td>
...
export async function printQuotation(data) {
  try {
    ...
    list.forEach(i => {
      const contentItem = `<tr><td>${i.name}</td><td><p>Moisture:${i.others.Moisture}</p><p>Admixture: ${i.others.Moisture}</p><p>Length: ${
        i.others.Length
      }</p><p> ${i.others.Fungus}</p><p> ${i.others.Color}</p></td><td>${i.pricePolicy.costPrice}</td><td>${i.others.Portofdestination}</td><td>${
        i.others.Packing
...
function checkTotalMoney(array) {
  let result;
  if (array) {
    let count = 0;
    let [item = {}] = array;
...
let totalPriceWater = 0;
const displayValue = (totalValue, { maxkWh, minkWh, pricekWh }) => {
  if (totalValue - maxkWh > minkWh) {
    totalPriceWater += (maxkWh - minkWh) * pricekWh;
    return (maxkWh - minkWh) * pricekWh;
  }
  ...
  if (totalValue - maxkWh <= 0) {
    totalPriceWater += totalValue - minkWh > 0 ? (totalValue - minkWh) * pricekWh : 0;
    return totalValue - minkWh > 0 ? (totalValue - minkWh) * pricekWh : 0;
  }
  return 0;
};

function displayPriceLadder(waterPricePolicy, value) {
  if (waterPricePolicy) {
    return waterPricePolicy.map(
      (price, index) =>
        `<tr>
          <td/>
          <td> Từ ${price.minkWh}m3 - ${price.maxkWh}m3</td>
          <td/>
          <td> ${price.pricekWh}</td>
          <td> 
          ${
            waterPricePolicy && index === waterPricePolicy.length - 1
              ? `${value >= price.maxkWh ? displayValue(value, { ...price, maxkWh: value }) : displayValue(value, price)}`
              : `${displayValue(value, price)}`
          }
          </td>
          <td/>
        </tr>`,
...
function displayPriceLadderWater(waterPricePolicy, number, data) {
  try {
    const mapdata = datas => {
      switch (datas) {
        case 0:
          return data.waterNumber0to10;
          break;
        case 10:
          return data.waterNumber10to20;
          break;
        case 20:
          return data.waterNumber20to30;
          break;
        case 30:
          return data.waterNumberGt30;
      }
    };
    ...
    const mapdataPrice = (datas, price) => {
      switch (datas) {
        case 0:
          return numberWithCommas(data.waterNumber0to10 * price);
          break;
        case 10:
          return numberWithCommas(data.waterNumber10to20 * price);
          break;
        case 20:
          return numberWithCommas(data.waterNumber20to30 * price);
          break;
        case 30:
          return numberWithCommas(data.waterNumberGt30 * price);
      }
    };
    if (Array.isArray(waterPricePolicy) && waterPricePolicy.length > 0) {
      return waterPricePolicy
        .map(
          (price, index) =>
            `<tr>
          <td/>
          <td> Từ ${price.minkWh}m3 - ${price.maxkWh}m3</td>
          <td>${mapdata(price.minkWh)}</td>
          <td> ${price.pricekWh.toLocaleString('es-AR', { maximumFractionDigits: 0 })}</td>
          <td>${mapdataPrice(price.minkWh, price.pricekWh)} </td>
          <td/>
        </tr>`,
        )
        .join('');
...
export async function printFee(data) {
  const currMonth = moment(data.toDate).get('month');
  const currYear = moment(data.toDate).get('year');
  const { carCharge = [], waterCharge = [] } = data;
  function numberWithCommas(x) {
    return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
  }
  const formatCost = data => {
    if (Number(data.taxVAT) === 0 && Number(data.taxEnv) === 0) {
      return data.waterCharge &&
        data.waterCharge[0] &&
        !isNaN(data.waterCharge[0].totalMoney || totalPriceWater) &&
        Number(data.waterCharge[0].totalMoney || totalPriceWater).toLocaleString('es-AR', {
        ...
        : '';
    } else if (Number(data.taxVAT) !== 0 && Number(data.taxEnv) === 0) {
      return numberWithCommas(data.taxVAT * 10);
    } else if (Number(data.taxVAT) === 0 && Number(data.taxEnv) !== 0) {
      return numberWithCommas(data.taxEnv * 10);
    } else {
      return numberWithCommas((data.taxEnv * 10).toFixed());
    }
  };

  const priceLadderList = (waterCharge && waterCharge[0] && waterCharge[0].priceLadderList) || [];

  let newCarCharge = [];
  carCharge.map(vehicle => {
    const findIndex = newCarCharge.findIndex(f => f.tags === vehicle.tags && f.unit === vehicle.unit);
    if (findIndex !== -1) {
      newCarCharge[findIndex].amount = Number(newCarCharge[findIndex].amount) + Number(vehicle.amount);
    } else {
      newCarCharge.push({
  ...
  newCarCharge.sort((a, b) => a.tags.localeCompare(b.tags));
  const fromValueWater =
    waterCharge && waterCharge.length === 1 && waterCharge[0].assetId && waterCharge[0].assetId.length === 1
      ? Number(waterCharge.fromValue) > 0
        ? Number(waterCharge.fromValue)
        : waterCharge[0].updateAsset &&
          waterCharge[0].updateAsset[0] &&
          waterCharge[0].updateAsset[0].fromValue &&
          Number(waterCharge[0].updateAsset[0].fromValue) > 0
          ? Number(waterCharge[0].updateAsset[0].fromValue)
          : ''
      : 0;
  const toValueWater =
    waterCharge && waterCharge.length === 1 && waterCharge[0].assetId && waterCharge[0].assetId.length === 1
      ? Number(waterCharge.toValue) > 0
        ? Number(waterCharge.toValue)
        : waterCharge[0].updateAsset &&
          waterCharge[0].updateAsset[0] &&
          waterCharge[0].updateAsset[0].toValue &&
          Number(waterCharge[0].updateAsset[0].toValue) > 0
          ? Number(waterCharge[0].updateAsset[0].toValue)
          : ''
      : 0;
  const totalValueWater =
    waterCharge && waterCharge.length === 1
      ? Number(waterCharge[0].totalValue) > 0
        ? Number(waterCharge[0].totalValue)
        : Number(data.waterValue) > 0
          ? Number(data.waterValue)
          : ''
      : 0;
  const priceList = numberWithCommas(totalValueWater * (data.waterCharge && data.waterCharge[0] && data.waterCharge[0].costPrice));
  const waterPriceLadderList =
    Array.isArray(priceLadderList) && priceLadderList.length > 0
      ? displayPriceLadderWater(priceLadderList, 100, data)
      : `<div>${data.waterCharge && data.waterCharge[0] && data.waterCharge[0].costPrice}</div>`;

  try {
    let html = `<table style="table-layout: fixed;"><tbody>
        ...
        <td style="font-weight: bold">Phí xe kỳ trước chưa thanh toán</td>
        <td></td>
        <td></td><td>${
          !isNaN(data.prevCarChargeMoney) &&
          Number(data.prevCarChargeMoney).toLocaleString('es-AR', {
            maximumFractionDigits: 0,
          }) !== '0'
            ? Number(data.prevCarChargeMoney).toLocaleString('es-AR', {
                maximumFractionDigits: 0,
      ...
      <td style="font-weight: bold">Phí giữ xe tháng ${data.period}</td>
      <td></td>
      <td></td>
      <td>${
        !isNaN(data.carChargeMoney) &&
        Number(data.carChargeMoney).toLocaleString('es-AR', {
          maximumFractionDigits: 0,
        }) !== '0'
          ? Number(data.carChargeMoney).toLocaleString('es-AR', {
              maximumFractionDigits: 0,
      ...
      <td></td>
         ...
         ${Array.isArray(newCarCharge) &&
           newCarCharge.map(
             (item, index) =>
               `<tr>
          <td></td>
          <td>${item.tags || 'XE ' + (index + 1)}</td>
          <td>${item.amount}</td>
          <td>${
            !isNaN(item.price) && Number(item.price).toLocaleString('es-AR', { maximumFractionDigits: 0 }) !== '0'
              ? Number(item.price).toLocaleString('es-AR', { maximumFractionDigits: 0 })
              : ''
          }</td>
          <td>${
            !isNaN(item.totalMoney) &&
            (Number(item.totalMoney) * Number(item.amount)).toLocaleString('es-AR', {
              maximumFractionDigits: 0,
            }) !== '0'
              ? (Number(item.totalMoney) * Number(item.amount)).toLocaleString('es-AR', {
                  maximumFractionDigits: 0,
        ...
        <td style="font-weight: bold">Phí sử dụng điện kỳ trước chưa thanh toán</td>
        <td></td>
        <td></td>
        <td>${
          !isNaN(data.prevElectricityMoney) &&
          Number(data.prevElectricityMoney).toLocaleString('es-AR', {
            maximumFractionDigits: 0,
          }) !== '0'
            ? Number(data.prevElectricityMoney).toLocaleString('es-AR', {
                maximumFractionDigits: 0,
      ...
      <td style="font-weight: bold">Phí sử dụng điện tháng ${data.period - 1 !== 0 ? data.period - 1 : 12} năm ${
      data.period <= 1 ? data.year - 1 : data.year
    }</td>
      <td>${
        data.electricityCharge && !isNaN(data.electricityCharge.amount) && Number(data.electricityCharge.amount) > 0
          ? Number(data.electricityCharge.amount).toLocaleString('es-AR', {
              maximumFractionDigits: 0,
            }) !== '0'
            ? Number(data.electricityCharge.amount).toLocaleString('es-AR', {
                maximumFractionDigits: 0,
      ...
      <td></td>
      <td>${
        data.electricityCharge && data.electricityCharge[0] && !isNaN(data.electricityCharge[0].totalMoney)
          ? Number(data.electricityCharge[0].totalMoney).toLocaleString('es-AR', {
              maximumFractionDigits: 0,
            }) !== '0'
            ? Number(data.electricityCharge[0].totalMoney).toLocaleString('es-AR', {
                maximumFractionDigits: 0,
      ...
      <td style="font-weight: bold">Phí sử dụng nước kỳ trước chưa thanh toán</td>
      <td></td>
      <td></td>
      <td>${
        !isNaN(data.prevWaterChargeMoney)
          ? Number(data.prevWaterChargeMoney).toLocaleString('es-AR', {
              maximumFractionDigits: 0,
            }) !== '0'
            ? Number(data.prevWaterChargeMoney).toLocaleString('es-AR', {
                maximumFractionDigits: 0,
      ...
      <td style="font-weight: bold">Phí sử dụng nước tháng ${data.period - 1 !== 0 ? data.period - 1 : 12} năm ${
      data.period <= 1 ? data.year - 1 : data.year
    }</td>
      <td></td>
      <td></td>
      ...
      <td>- Chỉ số đầu kỳ</td>
            ${
              data.waterCharge &&
              data.waterCharge.length === 1 &&
              data.waterCharge[0] &&
              data.waterCharge[0].assetId &&
              data.waterCharge[0].assetId.length === 1
                ? `<td>${fromValueWater}</td>`
                : `<td></td>`
            }
      <td></td>
      <td></td>
      ...
      ${data.waterCharge && data.waterCharge.length === 1 ? `<td>${totalValueWater}</td>` : `<td></td>`}
      <td>${waterPriceLadderList}</td>
      <td></td>
      <td></td>
      </tr><tr>
            <td></td>
            ...
            <td></td>
            <td></td>
            <td>${formatCost(data)}</td>
            <td></td>
        </tr>
        <tr>
            <td></td>
            <td>Thuế GTGT ${data.waterCharge && data.waterCharge[0] && Number(data.waterCharge[0].vat) > 0 ? data.waterCharge[0].vat : 5}%</td>
            <td></td>
            <td></td>
            <td>${
              !isNaN(data.taxVAT) && Number(data.taxVAT).toLocaleString('es-AR', { maximumFractionDigits: 0 }) !== '0'
                ? Number(data.taxVAT).toLocaleString('es-AR', { maximumFractionDigits: 0 })
                : ''
            }</td>
            <td></td>
        </tr>
        <tr>
            <td></td>
            <td>Phí bảo vệ môi trường ${
              data.waterCharge && data.waterCharge[0] && Number(data.waterCharge[0].vatEnv) > 0 ? data.waterCharge[0].vatEnv : 10
            }%</td>
            <td></td>
            <td></td>
            <td>${
              !isNaN(data.taxEnv) && Number(data.taxEnv).toLocaleString('es-AR', { maximumFractionDigits: 0 }) !== '0'
                ? Number(data.taxEnv).toLocaleString('es-AR', { maximumFractionDigits: 0 })
                : ''
            }</td>
            <td></td>
        </tr>
        <tr>
            ...
            <td style="font-weight: bold">Phí dịch vụ kỳ trước chưa thanh toán</td>
            <td></td>
            <td></td>
            <td>${
              !isNaN(data.prevServiceChargeMoney) &&
              Number(data.prevServiceChargeMoney).toLocaleString('es-AR', {
                maximumFractionDigits: 0,
              }) !== '0'
                ? Number(data.prevServiceChargeMoney).toLocaleString('es-AR', {
                    maximumFractionDigits: 0,
        ...
        <tr>
            ...
            <td style="font-weight: bold">Phí dịch vụ tháng ${data.period}</td>
            <td></td>
            <td></td>
            <td>${
              !isNaN(data.serviceChargeMoney) &&
              Number(data.serviceChargeMoney).toLocaleString('es-AR', {
                maximumFractionDigits: 0,
              }) !== '0'
                ? Number(data.serviceChargeMoney).toLocaleString('es-AR', {
                    maximumFractionDigits: 0,
        ...
        <tr>
            ...
            <td></td>
            <td></td>
            <td style="font-weight: bold;">${(!isNaN(Number(data.totalDebt)) &&
              Number(data.totalDebt).toLocaleString('es-AR', { maximumFractionDigits: 0 })) ||
              0}</td>
            <td></td>
        </tr>
        <tr>
            ...
            <td></td>
            <td>${
              !isNaN(data.totalPaid) && Number(data.totalPaid).toLocaleString('es-AR', { maximumFractionDigits: 0 }) !== '0'
                ? Number(data.totalPaid).toLocaleString('es-AR', { maximumFractionDigits: 0 })
                : ''
            }</td>
            <td></td>
        </tr>
        <tr>
            <td></td>
            <td style="font-weight: bold;">SỐ TIỀN CÒN PHẢI THANH TOÁN</td>
            <td></td>
            <td></td>
            <td>${
              (!isNaN(Number(data.totalDebt)) &&
                (Number(data.totalDebt) - Number(data.totalPaid)).toLocaleString('es-AR', {
                  maximumFractionDigits: 0,
                })) > 0
                ? (Number(data.totalDebt) - Number(data.totalPaid)).toLocaleString('es-AR', {
                    maximumFractionDigits: 0,
...
export async function printAllFee(data) {
  let html = ``;
  data &&
    data.map(fee => {
      html += printFee(fee);
...
export async function printFeeSamePrice(data) {
  const currMonth = moment(data.toDate).get('month');
  const currYear = moment(data.toDate).get('year');
  try {
    let html = `
    <table style = "table-layout: fixed;" >
      <tbody>
        ...
        <tr>
          <td style="text-align: right;">2</td>
          <td style="font-weight: bold;">Phí giữ xe</td>
          <td></td>
          <td></td>
          <td>${data.prevCarChargeMoney}</td>
          <td></td>
        </tr>
        <tr>
          <td></td>
        ...
        <tr>
          ...
          <td></td>
          <td></td>
        <td>${data.prevElectricityMoney}</td>
          <td></td>
        </tr>
        <tr>
          <td style="text-align: right;">4</td>
          <td style="font-weight: bold;">Phí sử dụng điện tháng ${currMonth}</td>
          <td>${data.electricityCharge.amount}</td>
          <td></td>
          <td>${data.electricityCharge.totalMoney}</td>
          <td></td>
        </tr>
        <tr>
          <td style="text-align: right;">5</td>
          ...
          <td></td>
          <td></td>
       <td>${data.prevWaterChargeMoney}</td>
          <td></td>
        </tr>
        <tr>
          <td style="text-align: right;">6</td>
          <td style="font-weight: bold;">Phí sử dụng nước tháng ${currMonth}</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        ...
        <tr>
          <td></td>
          <td>- Chỉ số đầu kỳ</td>
          <td>${data.waterCharge.fromValue}</td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td></td>
          <td>- Chỉ số cuối kỳ</td>
          <td>${data.waterCharge.toValue}</td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        ...
        <tr>
          ...
          <td></td>
          <td></td>
          <td>${data.waterCharge.totalMoney}</td>
          <td></td>
        </tr>
        <tr>
          <td></td>
        ...
        <tr>
          <td></td>
          <td>Phí bảo vệ môi trường 10%</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        ...
        <tr>
          <td style="text-align: right;">8</td>
          <td style="font-weight: bold;">Phí dịch vụ tháng ${currMonth}/${currYear}</td>
          <td></td>
          <td></td>
...
&quot;Times New Roman&quot;; mso-bidi-font-family:Calibri; mso-bidi-theme-font:minor-latin; color:black; mso-font-width:105%; font-size: medium; font-family: 'Times New Roman', Times,serif;">TOTAL</span></b></p>
    ...
    list.forEach((i, idx) => {
      ...
      <td><p style="max-width:500px">Tên sản phẩm: <b>${i.name}</b> <br> Nhãn Hiệu: <b>${i.tags}</b> <br> Kích thước: <b>${
        i.size
