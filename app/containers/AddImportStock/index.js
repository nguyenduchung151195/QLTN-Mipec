...
import { API_ADD_NEW_PRODUCT, API_SALE, API_SUPPLIERS, API_ORDER_PO, API_STOCK_EXPORT, API_ASSET, API_APPROVE_GROUPS } from '../../config/urlConfig';
...
export class addImportStock extends React.Component {
  ...
  state = {
    ...
    history: false,
    supplier: null,
    type: 2, // 0 đơn đặt hàng, 1 đặt hàng PO, 2 nhập hàng, 3 Chuyển kho,
    products: [],
    paidList: [],
    isFinished: true,
    orderId: undefined,
  ...
  componentDidMount() {
    if (!this.state.currentStock && this.props.dashboardPage.currentStock)
      this.setState({ currentStock: this.props.dashboardPage.currentStock, allOrg: this.props.dashboardPage.allOrg });
    this.loadOptionsFirst(API_SUPPLIERS, 'supplierList');
  }

  componentDidUpdate(props, state) {
    // set lại code === '' để render lại code khi chuyển từ type ===3 hoặc 7 sang các type khác do type 3 hoặc 7 lấy từ type của yêu cầu xuât kho
    // còn code của các type khác là tự sinh
    if (state.type !== this.state.type && (state.type === 3 || state.type === 7)) {
      this.setState({
        code: '',
      });
    }
  ...
  componentWillReceiveProps(props, nextState) {
    if (props.dashboardPage.currentStock !== '') {
      ...
      this.state.allOrg = props.dashboardPage.allOrg || [];
    }
    if (this.props.addImportStock !== props.addImportStock) {
      ...
      if (this.state.isEditPage && props.addImportStock.order) {
        ...
        if (list.length > 0 && products && products.length > 0) {
          const newList = products.map(item => {
            ...
            item.catalog = x && x.catalog;
            item.code = '';

            let totalAmount = 0;
            if (x) {
              item.code = x.code || '';
              const stock = toStock && Array.isArray(x.sellingPoint) && x.sellingPoint.find(a => a.organizationUnitId === toStock.organizationUnitId);
              if (stock) {
      ...
      if (this.state.isEditPage && code && props.addImportStock.order !== this.props.addImportStock.order) {
        // if (type !== 3 || type !==7) {
        //   this.props.onChangeStock(toStock && toStock.organizationUnitId); //===> set lai kho toi
        // }
        this.setState({
          isFinished,
          name,
  ...
  render() {
    ...
    const { classes, addImportStock, intl, dashboardPage } = this.props;
    const { discount, discountPercent, paymentAmount, name, code, note, paidList } = this.state;

    const { allTemplates } = dashboardPage;
    const moduleTemplates = (allTemplates || []).filter(t => t.moduleCode === 'StockImport');
            ...
            <Paper className={classes.paper}>
              <div style={{ display: 'flex' }}>
                ...
                >
                  ...
                  <MenuItem value={2}>Nhập hàng trực tiếp-theo sản phẩm</MenuItem>
                  <MenuItem value={6}>Nhập hàng trực tiếp-theo tài sản</MenuItem>
                  <MenuItem value={3}>Phê duyệt chuyển kho-theo sản phẩm</MenuItem>
                  <MenuItem value={7}>Phê duyệt chuyển kho-theo tài sản</MenuItem>
                </TextField>
              </div>
            </Paper>
          </Grid>
              ...
              <Paper className={classes.paper}>
                <Table>
                  <TableHead>
                    ...
                    {this.state.type !== 3 && this.state.type !== 6 && this.state.type !== 7 ? (
                      <React.Fragment>
                        ...
                        <TableRow key={item._id}>
                          ...
                          {this.state.type !== 3 && this.state.type !== 6 && this.state.type !== 7 ? (
                            <React.Fragment>
                              ...
                              {this.state.type === 3 || this.state.type === 7 ? (
                                ...
                                <TableCell style={{ padding: '0 10px', width: '200px' }}>
                                  <TextValidator
                                    ...
                                    type="number"
                                    validators={[
                                      `minNumber:${this.state.isEditPage === false
                                        ? 0
                                        : item.amountBackup - item.amount > item.totalAmount
                                          ? item.totalAmount
                                          : 0
                                      }`,
                                      `${`maxNumber:${this.state.isEditPage === false
                                    ...
                                    errorMessages={[
                                      ...
                                      `Giới hạn nhập ${this.state.isEditPage === false
                                        ? item.totalDelivery - item.delivered
                                        : item.totalDelivery - item.delivered + item.amountBackup
                                      }`,
                                    ]}
                              ...
                              <TableCell style={{ padding: '0 10px' }}>
                                {!isNaN(Number(totalPrice)) && Number(totalPrice).toLocaleString('es-AR', { maximumFractionDigits: 0 })}
                              </TableCell>
                            </React.Fragment>
                          ) : (
                            <React.Fragment>
            ...
            <Grid item md={4}>
              {this.state.type !== 3 && this.state.type !== 7 ? (
                  ...
                  {this.state.type !== 6 && (
                    <>
                      ...
                      <TextField
                        label="Tổng tiền"
                        value={
                          this.state.total && !isNaN(this.state.total) ? this.state.total.toLocaleString('es-AR', { maximumFractionDigits: 0 }) : 0
                        }
                        variant="outlined"
                        margin="normal"
                        style={{ width: '100%', zIndex: 0 }}
                      ...
                      <TextField
                        ...
                        value={
                          this.state.total && !isNaN(this.state.remaining)
                            ? this.state.remaining.toLocaleString('es-AR', { maximumFractionDigits: 0 })
                            : 0
                        }
                        variant="outlined"
                        margin="normal"
                            ...
                            <TableRow>
                              ...
                              <TableCell>
                                {item.amount &&
                                  ...
                                  Number(item.amount).toLocaleString('es-AR', { maximumFractionDigits: 0 })}
                              </TableCell>
                              <TableCell>
                ...
                <Paper className={classes.paper}>
                  ...
                  {(this.state.type === 3 || this.state.type === 7) && this.state.isApproved === true && this.state.isEditPage === false ? (
                    <React.Fragment>
                      <Typography
                        style={{
                          color: 'grey',
                  ...
                  <TextField
                    ...
                    margin="normal"
                  >
                    {Array.isArray(this.props.dashboardPage.allStock) &&
  ...
  handleSubmitForm = () => {
    ...
    // const allStock= this.props.addImportStock && this.props.addImportStock.allStock||[];
    // const x = allOrg && allOrg.find(item => item.id === currentStock);
    const allStock = this.props.dashboardPage && this.props.dashboardPage.allStock ? this.props.dashboardPage.allStock : [];
    const x = allStock && allStock.find(item => item.id === currentStock);
    ...
    let fromStockRaw;
    if (type === 3 || type === 7) {
      // const y = allOrg.find(item => item.id === toStock);
      // const z = allOrg.find(item => item.id === fromStock);
    ...
    //   this.props.onChangeSnackbar({ status: true, message: 'Nhà cung cấp không được để trống', variant: 'error' }); // tạm thời comment đợi thống nhất với khách: comment lúc 8/12/2021
    //   return;
    // }
    ...
    let state = 0;
    if (type !== 3 && type !== 7 && !this.state.isFinish) {
      state = 4;
    }
    if (type !== 3 && type !== 7 && this.state.isFinish === true && !this.state.isApproved) {
      state = 3;
    }
    if ((type === 3 || type === 7) && !this.state.isApproved) {
      state = 3;
    }
    ...
    const body = {
      ...
      // toStock: this.state.sale && this.state.sale.toStock?this.state.sale.toStock: {},
      // fromStock: this.state.sale && this.state.sale.fromStock?this.state.sale.fromStock: {},
      state,
      ...
      state: state,
      dynamicForms: this.state.dynamicForms,
      dynamicFormsList: this.props.dashboardPage && this.props.dashboardPage.allTemplates ? this.props.dashboardPage.allTemplates : [],
      groupApproved,
    ...
    // if (supplier == null && type !== 3) {
    //   this.props.onChangeSnackbar({ status: true, message: 'Nhà cung cấp không đượcc để trống', variant: 'error' }); //Tạm thời comment đợi thống nhất với khách: comment lúc 8/12/2021
    //   return;
    // }
    if (products.length === 0) {
      this.props.onChangeSnackbar({ status: true, message: 'Sản phẩm không được để trống', variant: 'error' });
    ...
    if (this.state.isEditPage) {
      ...
      const { addImportStock } = this.props;
      const state = addImportStock.order && addImportStock.order.state;
      // if (Number(state) === 4)
      // return this.props.onChangeSnackbar({ status: true, message: 'Không thể xóa các yêu cầu đã hoàn thành!', variant: 'error' });
  ...
  handleAddProductImport = product => {
    ...
    if (!products.find(item => item._id === product._id)) {
      const stock =
        product &&
        Array.isArray(product.sellingPoint) &&
        product.sellingPoint.find(item1 => item1.organizationUnitId === this.props.dashboardPage.profile.workingOrganization._id);
      let totalAmount;
      if (stock) {
        totalAmount = stock.amount;
      } else {
  ...
  handleAddAssetImport = product => {
    ...
    if (!products.find(item => item._id === product._id)) {
      ...
      if (this.state.type === 6 && this.state.type === 7) {
        products.push({
          ...product,
          amount: 1,
          totalAmount,
  ...
  handleAddProductOfStock = sale => {
    if (sale.isImportedStock) {
      ...
      const listId = Array.isArray(products) && products.map(item => item.productId);
      const params = {
        filter: {
          _id: { $in: listId },
        },
  ...
  loadOptions = (newValue, callback, api, type, state) => {
    const token = localStorage.getItem('token');
    let filter = {};
    if (type === 3 || type === 7) {
      filter = {
    ...
    // const url = `${api}?filter%5Bname%5D%5B%24regex%5D=${newValue}&filter%5Bname%5D%5B%24options%5D=gi${api === API_STOCK_EXPORT ? `&filter%5Btype%5D=${type}` : ''
    //   }`;
    // return fetch(url, {
    //   headers: {
    //     Authorization: `Bearer ${token}`,
    //   },
    // })
    return fetch(`${api}?${serialize(filter)}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
  ...
  loadFindOption = () => {
    ...
    switch (type) {
      ...
      case 6:
        api = API_ASSET;
        functionHandle = this.handleAddAssetImport;
        break;
      case 7:
        api = API_STOCK_EXPORT;
        // functionHandle = this.handleAddProductImport;
    ...
    return (
      <AsyncSelect
        ...
        loadOptions={(newValue, callback) => this.loadOptions(newValue, callback, api, type, state)}
        loadingMessage={() => 'Đang tải ...'}
        components={{ Option, SingleValue }}
        // onBlur={() => field.onBlur({ value: field.value })}
