...
import {resetLtAccount, getLtAccount, createAccountRequest, updateAccountRequest, updatePasswordRequest, mergeAccountRequest } from './actions';

/* eslint-disable react/prefer-stateless-function */
function AddLtAccountPage(props) {
    // const [localState, setLocalState] = useState({accountStatus: '1'})
    const [localState, setLocalState] = useState({accountStatus: '1', moduleCode: "Contract"})
    ...
    const ltAccount = viewConfig.find(item => item.code ==='LtAccount')
    const accountStatus = ltAccount && ltAccount.listDisplay.type.fields.type.columns.find(item => item.name ==='accountStatus')
    const menuItem = accountStatus && accountStatus.menuItem;
    ...
    // );
    useEffect(()=>{
        setLocalState({...localState, ...props.accountRequest.data})
    }, [props.accountRequest.data])
    useEffect(()=>{
        if(id && id!=="add"){
            props.onGetLtAccount(id)
        }
        else{
            props.onResetLtAccount();
        }
    }, [id])
    ...
    useEffect(()=>{
        if(createAccountRequestSuccess === true){
            props.onMergeData({createAccountRequestSuccess: null})
            props.history.goBack();
        }
    }, [createAccountRequestSuccess])
    useEffect(()=>{
        if(updateAccountRequestSuccess === true){
            props.onMergeData({updateAccountRequestSuccess: null})
            props.history.goBack();
        }
    ...
    useEffect(()=>{
        if(updatePasswordRequestSuccess === true){
            props.onMergeData({updatePasswordRequestSuccess: null})
            props.history.goBack();
        }
...
function mapDispatchToProps(dispatch) {
    return {
        dispatch,
        onResetLtAccount: ()=>dispatch(resetLtAccount()),
        onGetLtAccount: (id)=>dispatch(getLtAccount(id)),
        ...
        onMergeData:(data)=>dispatch(mergeAccountRequest(data)),
    };
}
