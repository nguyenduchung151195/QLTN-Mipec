...
const switchRoutes = listRole => (
  <Switch>
    {dashRoutes.map(prop => {
      ...
      if ((x.length === 1 && x[0] !== '') || (x.length !== 1 && x[1])) {
        if (prop.redirect) return <Redirect from={prop.path} to={prop.pathTo} key={prop.path} />;
        if (prop.collapse) {
          const menu = [<Route exact path={prop.path} component={prop.component} key={prop.path} />];
          const submenu = prop.views.map(prop => <Route exact path={prop.path} component={prop.component} key={prop.path} />);
...
class Dashboard extends React.Component {
  ...
  handleChangeStock = e => {
    const allOrg = this.props.dashboardPage.allStock || [];
    const stockChoose = allOrg.find(value => value.id === e.target.value);
  ...
  render() {
    ...
    if (role.roles) {
      role.roles.forEach(item => {
        if (
          (item.codeModleFunction === 'Contract' && item.methods.find(method => method.name === 'GET').allow === true) ||
          (item.codeModleFunction === 'ContractSupper' && item.methods.find(method => method.name === 'GET').allow === true)
        ) {
          codeModleFunctionAllowForFunction.push('Contract');
        } else if (item.methods.find(method => method.name === 'GET').allow) {
    ...
    if (
      ...
      !listPath.includes('tower') &&
      !listPath.includes('Guarantee') &&
    ...
    return (
      <div className={classes.wrapper}>
        ...
        <Sidebar
          ...
          logoDisplayText={this.state.displayName}
