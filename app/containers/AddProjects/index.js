...
function KanbanStep(props) {
  const { kanbanStatus, id, checkStep } = props;
  ...
  return (
    <React.Fragment>
      ...
      <VisibilityOffRounded
        ...
        style={{ fontSize: '0,5rem', marginLeft: 5 }}
      />
      <Menu anchorEl={anchorEl} onClose={() => setAnchorEl(null)} open={Boolean(anchorEl)}>
        {planPeople.map(i => (
          <MenuItem>
            ...
            <span
              style={{ color: people.map(it => it._id).includes(i._id) ? 'transparent' : 'red' }}
            >
              {i.name}
            </span>
          </MenuItem>
...
export class AddProjects extends React.Component {
  constructor(props) {
    super(props);
    const taskColumns = JSON.parse(localStorage.getItem('viewConfig')).find(
      item => item.code === 'Task',
    ).listDisplay.type.fields.type.columns;
    const names = {};
    ...
    const currentStatus = listCrmStatus
    ...
    this.state = {
      ...
      checkStep: false,
      roleGroupSource: '',
      checkProgress: 'XÁC NHẬN HOÀN THÀNH CÔNG VIỆC',
      disabled: false,
      listJoinTemplate: [],
      checkReload: true,
    };
  ...
  componentDidMount() {
    ...
    let { addProjects } = this.props;
    fetchData(`${API_ROLE_GROUP}?clientId=MIPEC`)
      .then(response => {
        this.setState({
          dataRoleCn: response.data,
          dataRoleCn2: response.data,
          dataRoleTg: response.data,
        });
      })
      .catch();
    fetchData(API_USERS)
    ...
    if (id !== 'add') {
      const { listJoin } = addProjects;
      if (listJoin) {
        let newListJoin = [];
        Array.isArray(listJoin) &&
          listJoin.map(item => {
            newListJoin.push(item._id);
          });
        this.setState({
          checkReload: false,
          listJoinTemplate: newListJoin,
        });
      }
    }
    if (addProjects && addProjects.kanbanStatus === '4') {
      this.setState({ checkStep: true });
    }
  ...
  componentWillReceiveProps(props) {
    if (
      !!props.dashboardPage.roleTask.roles &&
      props.dashboardPage.roleTask.roles !== undefined &&
      props.dashboardPage.roleTask.roles.length > 0
    ) {
      this.setState({
        taskManagerRole: props.dashboardPage.roleTask.roles
          .find(elm => elm.code === 'BUSSINES')
          .data.find(elm => elm.name === 'taskManager').data,
        taskInCharge: props.dashboardPage.roleTask.roles
          .find(elm => elm.code === 'BUSSINES')
          .data.find(elm => elm.name === 'inCharge').data,
        taskSupport: props.dashboardPage.roleTask.roles
          .find(elm => elm.code === 'BUSSINES')
          .data.find(elm => elm.name === 'support').data,
        taskViewable: props.dashboardPage.roleTask.roles
          .find(elm => elm.code === 'BUSSINES')
          .data.find(elm => elm.name === 'viewable').data,
        taskJoin: props.dashboardPage.roleTask.roles
          .find(elm => elm.code === 'BUSSINES')
          .data.find(elm => elm.name === 'join').data,
        taskApproved: props.dashboardPage.roleTask.roles
          .find(elm => elm.code === 'BUSSINES')
          .data.find(elm => elm.name === 'approved').data,
    ...
    if (
      props.addProjects.parentId &&
      this.props.addProjects.parentId &&
      props.addProjects.parentId !== this.props.addProjects.parentId
    ) {
      const { addProjects } = props;
      const idParent =
        addProjects && addProjects.parentId && addProjects.parentId._id
          ? addProjects.parentId._id
          : null;
      if (idParent) {
        const filter = {
          limit: 20,
          ...
          filter: {
            parentId: idParent,
            _id: { $ne: addProjects._id },
          },
        };
        fetch(`https://g.lifetek.vn:233/api/tasks/all? ${serialize(filter)}`, {
          method: 'GET',
          headers: {
          ...
          .then(res => res.json())
          .then(data => this.setState({ taskSameParent: data.data }));
      }
    }
    if (props.profile) {
      this.setState({
        roleGroupSource: props.profile && props.profile.roleGroupSource,
      });
    }
    if (props.addProjects.confirmFinishTask && props.addProjects.confirmFinishTask === true) {
      console.log(props.addProjects.confirmFinishTask);
      this.setState({ checkProgress: 'ĐÃ HOÀN THÀNH' });
    }
  }
  ...
  handleSelectSuggest = suggest => {
    ...
    this.props.mergeData({
      search: '',
      locationAddress: suggest.formatted_address,
      location: { lat, lng },
    });
  };
  ...
  selectTemplate = template => {
    ...
    if (template) {
      let newListJoinTemplate = [];
      let newIdListJoinTemplate = [];
      const { treeData } = template;
      console.log('treeData', treeData);
      treeData.length > 0 &&
        treeData.map(item => {
          let joinArray = item.join;
          joinArray.length > 0 &&
            joinArray.map(item => {
              if (item.id) {
                newListJoinTemplate.push(item.id);
                newIdListJoinTemplate.push({_id:item.id,...item})
              }
            });
        });
      
      let newList = [...new Map(newIdListJoinTemplate.map(item=>[item._id,item])).values()]
      if (newListJoinTemplate.length > 0) {
        this.setState({ checkReload: true });
        this.props.mergeData({ join: newList });
        setTimeout(() => {
          this.setState({ checkReload: false, listJoinTemplate: newListJoinTemplate});
        }, 1000);
      } else {
        this.setState({ listJoinTemplate: newListJoinTemplate, checkReload: false });
        this.props.mergeData({ join: null });
      }

      const { startDate } = this.props.addProjects;
      this.calculateEndate(startDate, template);
    } else {
      this.props.mergeData({ template: null });
      this.setState({ checkReload: true });
    }
  };
  ...
  calculateEndate = (startDate, template) => {
    ...
    this.props.mergeData({
      endDate: newEndDate,
      template,
      joinChild: joinChild.joins,
      startDate,
      joinChildData: joinChild.joinsData,
    });
  };
  ...
  handleOpenDialogProgress = () => {
    let checkRole = false;
    if (this.props.addProjects && this.props.profile) {
      Array.isArray(this.props.addProjects.inCharge) &&
        this.props.addProjects.inCharge.map(item => {
          if (item._id === this.props.profile._id || item === this.props.profile._id) {
            checkRole = true;
          }
        });
      Array.isArray(this.props.addProjects.taskManager) &&
        this.props.addProjects.taskManager.map(item => {
          if (item._id === this.props.profile._id || item === this.props.profile._id) {
            checkRole = true;
          }
        });
    ...
    if(this.props.addProjects.createdBy && this.props.addProjects.createdBy._id === this.props.profile._id){
        checkRole = true;
    }
  ...
  handleCheckProgress = () => {
    ...
    const checkManager = addProjects.taskManager.find(item => item && item._id === profile._id);
    if (addProjects.taskStatus == 3 || addProjects.taskStatus == 7) {
      if (addProjects.createdBy._id === profile._id || checkManager) {
        const data = { confirmFinishTask: true };
        const url = `${API_TASK_PROJECT}/confirmFinishTask/${addProjects._id}`;
        fetch(url, {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
        ...
        this.props.onChangeSnackbar({
          status: true,
          message:
            'Bạn không có quyền thay đổi trạng thái này , chỉ có người tạo và quản lí của bộ phận phòng ban trong công việc mới được xác nhận',
          variant: 'error',
        });
      }
    } else {
  ...
  onSave = () => {
    ...
    if (addProjects.join === null) {
      addProjects.join = [];
      console.log(addProjects.join);
    }
    const listJoin =
      addProjects.join &&
      addProjects.joinChild &&
      addProjects.joinChild.concat(addProjects.join.map(item => item._id));
    const join = [...new Set(listJoin)];
    if (addProjects.taskType == 2) {
      addProjects.taskType = 2;
    } else if (
      (addProjects.inCharge &&
        addProjects.inCharge[0] === undefined &&
        addProjects.viewable &&
        addProjects.viewable[0] === undefined &&
        addProjects.support &&
        addProjects.support[0] === undefined &&
        addProjects.taskManager &&
        addProjects.taskManager[0] === undefined &&
        addProjects.join &&
        addProjects.join[0] === undefined &&
        addProjects.approvedProgress === null) ||
      (addProjects.inCharge &&
        addProjects.inCharge[0] &&
        addProjects.inCharge[0]._id === addProjects.createdBy &&
        addProjects.createdBy._id &&
        addProjects.viewable &&
        addProjects.viewable[0] &&
        addProjects.viewable[0]._id === addProjects.createdBy &&
        addProjects.createdBy._id &&
        addProjects.support &&
        addProjects.support[0] &&
        addProjects.support[0]._id === addProjects.createdBy &&
        addProjects.createdBy._id &&
        addProjects.taskManager &&
        addProjects.taskManager[0] &&
        addProjects.taskManager[0]._id === addProjects.createdBy &&
        addProjects.createdBy._id &&
        addProjects.join &&
        addProjects.join[0] &&
        addProjects.join[0]._id === addProjects.createdBy &&
        addProjects.createdBy._id &&
        addProjects.approvedProgress &&
        addProjects.approvedProgress._id === addProjects.createdBy &&
        addProjects.createdBy._id)
    ) {
      console.log('taskType 3');
      addProjects.taskType = 3;
    } else {
      console.log('taskType', 1);
      addProjects.taskType = 1;
    }
    let data = {
      ...
      customer: this.props.customerBos
        ? this.props.customerBos.customerId
        : addProjects.customer
          ? addProjects.customer._id
          : null,
      ...
      approved: addProjects.approved
        ? addProjects.approved.map(item => ({ id: item.id ? item.id : item._id, name: item.name }))
        : [],
      ...
      join,
      taskType: addProjects && addProjects.taskType,
      ...
      businessOpportunities: this.props.businessOpportunities
        ? this.props.businessOpportunities
        : null,
      ...
      state: addProjects.approvedProgress === '' ? '3' : '0',
      // lat: geography.lat,
      // lon: geography.lon,
  ...
  handleSaveProject = () => {
    ...
    const { endDate, approvedProgress, customer, state } = addProjects;
    // if (!endDate) return;
  ...
  onSaveProgress = () => {
    ...
    const { taskSameParent } = this.state;
    const leafIndex = addProjects.leafIndex;
    const prevTaskSameParrent =
      Array.isArray(taskSameParent) &&
      taskSameParent.find(f => Number(f.leafIndex) === Number(leafIndex) + 1);
    if (prevTaskSameParrent) {
      const namePrevTaskSameParrent = prevTaskSameParrent.name;
      if (Number(prevTaskSameParrent.progress) < 100) {
        return this.props.onChangeSnackbar({
          status: true,
          message: `Bạn chưa hoàn thành công việc ${namePrevTaskSameParrent}`,
          variant: 'error',
        });
      }
    }
    ...
    const data = {
      ...
      progress: addProjects.selectStatus === 3 ? 100 : addProjects.selectProgress * 1,
  ...
  caculeDisable = value => {
    ...
    if ((selectStatus === 3 || selectStatus === 4 || selectStatus === 6) && selectStatus !== value)
      return true;
  ...
  selectTask = e => {
    ...
    let parentStatus;
    if (id === idSelect)
      parentStatus = this.props.addProjects.parentId
        ? this.props.addProjects.parentId.taskStatus
        : null;
    else {
  ...
  changeRatio = (index, value) => {
    const listRatio = this.props.addProjects.listRatio.map(
      (it, idx) => (idx === index ? { ...it, ratio: value } : it),
    );
  ...
  changeCostEstimate = (index, value) => {
    const listRatio = this.props.addProjects.listRatio.map(
      (it, idx) => (idx === index ? { ...it, costEstimate: value } : it),
    );
    this.props.mergeData({ listRatio });
  };
  ...
  mapFunctionDocument = item => {
    ...
    return {
      ...
      typeDocument: typeDocumentArr.find(elm => elm.value === item.typeDocument)
        ...
        : item.typeDocument,
      urgency: urgencyArr.find(elm => elm.value === item.urgency)
        ? urgencyArr.find(elm => elm.value === item.urgency).title
        : item.urgency,
      where: whereArr.find(elm => elm.value === item.where)
        ? whereArr.find(elm => elm.value === item.where).title
        : item.where,
      storage: storageArr.find(elm => elm.value === item.storage)
        ? storageArr.find(elm => elm.value === item.storage).title
        : item.storage,
      density: densityArr.find(elm => elm.value === item.density)
        ? densityArr.find(elm => elm.value === item.density).title
        : item.density,
    };
  };
  ...
  render() {
    ...
    const allData1 = dataRoleCn.concat(dataUser);
    const allData2 = dataRoleTg.concat(dataUser);
    ...
    const obligate =
      this.props.dashboardPage.roleTask.roles &&
      this.props.dashboardPage.roleTask.roles.find(item => item.code === 'SPECIALL');
    const roleModuleObligate = obligate && obligate.data ? obligate.data : [];
    const roleModuleObligateData =
      roleModuleObligate &&
      roleModuleObligate.find(elm => elm.name === 'obligate') &&
      roleModuleObligate.find(elm => elm.name === 'obligate').data;

    const roleCodeCalendar =
      this.props.dashboardPage &&
      this.props.dashboardPage.role &&
      this.props.dashboardPage.role.roles &&
      this.props.dashboardPage.role.roles.find(item => item.codeModleFunction === 'Calendar');
    const roleModuleCalendar =
      roleCodeCalendar && roleCodeCalendar.methods ? roleCodeCalendar.methods : [];

    const roleCodeDocumentary =
      this.props.dashboardPage &&
      this.props.dashboardPage.roleTask &&
      this.props.dashboardPage.roleTask.roles &&
      this.props.dashboardPage.roleTask.roles.find(
        item => item.codeModleFunction === 'Documentary',
      );
    const roleModuleDocumentary =
      roleCodeDocumentary && roleCodeDocumentary.methods ? roleCodeDocumentary.methods : [];
    ...
    const taskCreatedBy =
      bussines &&
      bussines.find(elm => elm.name === 'createdBy') &&
      bussines.find(elm => elm.name === 'createdBy').data;
    const taskOthers =
      bussines &&
      bussines.find(elm => elm.name === 'others') &&
      bussines.find(elm => elm.name === 'others').data;
    let isTaskManager =
      profile &&
      profile._id &&
      addProjects &&
      Array.isArray(addProjects.taskManager) &&
      addProjects.taskManager.find(t => t._id === profile._id)
        ? true
        : false;
    let isInCharge =
      profile &&
      profile._id &&
      addProjects &&
      Array.isArray(addProjects.inCharge) &&
      addProjects.inCharge.find(t => t._id === profile._id)
        ? true
        : false;
    let isSupport =
      profile &&
      profile._id &&
      addProjects &&
      Array.isArray(addProjects.support) &&
      addProjects.support.find(t => t._id === profile._id)
        ? true
        : false;
    let isViewable =
      profile &&
      profile._id &&
      addProjects &&
      Array.isArray(addProjects.viewable) &&
      addProjects.viewable.find(t => t._id === profile._id)
        ? true
        : false;
    let isJoin =
      profile &&
      profile._id &&
      addProjects &&
      Array.isArray(addProjects.join) &&
      addProjects.join.find(t => t._id === profile._id)
        ? true
        : false;
    let isCreatedBy =
      profile &&
      profile._id &&
      addProjects &&
      addProjects.createdBy &&
      addProjects.createdBy._id === profile._id
        ? true
        : false;
    ...
    const ButtonUI = props => (
      <Buttons
        onClick={() => this.handleChangeButton(props.tabIndex)}
    ...
    const dayProgress =
      ((new Date() - new Date(addProjects.startDate)) * 100) /
      (new Date(addProjects.endDate) - new Date(addProjects.startDate)).toFixed();
      ...
      return (
        <div>
            ...
            title={
              id === 'add'
                ? `${intl.formatMessage(
                    messages.themmoi || {
                      id: 'themmoi',
                      defaultMessage: 'Thêm mới công việc/Dự án',
                    },
                  )}`
                : `${intl.formatMessage(
                    messages.chinhsua || {
                      id: 'chinhsua',
                      defaultMessage: 'Cập nhật công việc/Dự án',
                    },
                  )}`
            }
            onGoBack={() => {
          ...
          <Grid container spacing={16}>
            <div style={{ width: '100%' ,paddingTop: id === 'add' ? '55px': '0'}}>
              <KanbanStep
                handleStepper={this.handleStepper}
                kanbanStatus={addProjects.kanbanStatus}
                currentStatus={currentStatus}
                id={this.props.id}
                checkStep={this.state.checkStep}
              />
            </div>
            ...
            <Grid item md={6} style={{ zIndex: 0 }}>
              ...
              <Grid md={12} style={{ marginTop: 10, display: 'flex' }}>
                <Grid item md={6} style={{ marginRight: 5 }}>
                  <MuiPickersUtilsProvider utils={MomentUtils}>
                    <DateTimePicker
                      ...
                      required
                      disabled={
                        (!addProjects.isSmallest || !canUpdateTaskPlan(addProjects, currentUser)) &&
                        id !== 'add'
                      }
                ...
                <Grid item md={6} style={{ marginLeft: 5 }}>
                  <MuiPickersUtilsProvider utils={MomentUtils}>
                    <DateTimePicker
                      ...
                      variant="outlined"
                      disabled={
                        ((!addProjects.isSmallest ||
                          !canUpdateTaskPlan(addProjects, currentUser)) &&
                          id !== 'add') ||
                        addProjects.template
                      }
                ...
                {roleModuleObligateData && roleModuleObligateData.access === true
                  ? id === 'add' && (
                      <span style={{ float: 'right', display: 'none', alignItems: 'center' }}>
                        <Checkbox
                          ...
                          disabled={
                            speciall &&
                            speciall.find(elm => elm.name === 'obligate').data.access !== true
                          }
                        />
                        Bắt buộc tham gia
                      </span>
              ...
              <AsyncAutocomplete
                ...
                customOptionLabel={option =>
                  `${option.name}${
                    option.apartmentCode && option.apartmentCode.length
                      ? ` - ${option.apartmentCode.join(', ')}`
                      : ''
                  }`
                }
            ...
            <Grid item md={6} style={{ zIndex: 0 }}>
              <Typography variant="h5">
                ...
                <label htmlFor="contained-button-file">
                  <Tooltip title="Thay đổi ảnh cover dự án">
                    <AddPhotoAlternate
                      style={{ cursor: 'pointer', float: 'right', color: '#3ea8fd' }}
                    />
                  </Tooltip>
                </label>
              </Typography>
              {addProjects.parentId ? (
                <Autocomplete
                  ...
                  suggestions={addProjects.employees}
                  customOptionLabel={option =>
              ...
              {this.state.checkReload ? (
                <Autocomplete
                  ...
                  onChange={value => {
                    ...
                    let { dataRoleCn2 } = this.state;
                    console.log(value, 'value');
                    if (!length) {
                      this.setState({ dataRoleTg: dataRoleCn2 });
                    }
                    if (length && value[length - 1].roles) {
                      let { join } = addProjects;
                      let dataJoin = join ? join : [];
                      let dataGroup = [];
                      this.state.dataUser.forEach(item => {
                        if (item.roleGroupSource === value[length - 1].code) {
                          dataGroup.push(item);
                          dataJoin.push(item);
                        }
                      });
                      const { dataRoleTg } = this.state;
                      console.log(dataRoleTg);
                      const data = _.cloneDeep(dataRoleTg);
                      const index = data.findIndex(item => item.name === value[length - 1].name);
                      data.splice(index, 1);
                      this.setState({ dataRoleTg: data });
                      this.props.mergeData({ join: dataJoin });
                    } else {
                      this.props.mergeData({ join: value });
                    }
                  }}
                  suggestions={allData2}
                  value={addProjects.join}
                />
              ) : (
                ''
              )}
              {/* {console.log(2009, this.state.listJoinTemplate.length)} */}
              {this.state.checkReload === false ? (
                <AsyncAutocomplete
                  // filters={['name', 'code']}
                  isDisabled={true}
                  ...
                  error={localMessages && localMessages.join}
                  helperText={localMessages && localMessages.join}
                  onChange={value => this.props.mergeData({ join: value === null ? [] : value })}
                  url={API_USERS}
                  listJoinTemplate={this.state.listJoinTemplate}
                  value={addProjects.join}
                  // styleListJoin={this.state.styleListJoin}
                  customOptionLabel={option =>
                    `${option.name}${option.code ? ` - ${option.code}` : ''}`
                  }
                  filter={{
                    _id: { $in: this.state.listJoinTemplate },
                  }}
                />
              ) : (
                ''
              ...
              <AsyncAutocomplete
                ...
                filter={{ status: 1, clientId: clientId }}
              />
              <AsyncAutocomplete
                // filters={['name', 'code']}
      ...
      <div className="project-main">
        ...
        <ValidatorForm onSubmit={this.handleSaveProject}>
                ...
                <div className="add-project">
                  ...
                  <TaskType
                    ...
                    description={intl.formatMessage(
                      messages.tieudebaomat || {
                        id: 'tieudebaomat',
                        defaultMessage: 'tieudebaomat',
                      },
                    )}
                    title={intl.formatMessage(
                      messages.nhombaomat || { id: 'nhombaomat', defaultMessage: 'nhombaomat' },
                    )}
                  ...
                  <TaskType
                    ...
                    description={intl.formatMessage(
                      messages.tieudecongkhai || {
                        id: 'tieudecongkhai',
                        defaultMessage: 'tieudecongkhai',
                      },
                    )}
                    title={intl.formatMessage(
                      messages.nhomcongkhai || {
                        id: 'nhomcongkhai',
                        defaultMessage: 'nhomcongkhai',
                      },
                    )}
                  />
                  {speciall &&
                  speciall.find(elm => elm.name === 'addSpecial').data.access === true ? (
                    <TaskType
                      ...
                      description={intl.formatMessage(
                        messages.tieudecongkhai || { id: 'tieudean', defaultMessage: 'tieudean' },
                      )}
                      title={intl.formatMessage(
                        messages.nhoman || { id: 'nhoman', defaultMessage: 'nhoman' },
                      )}
                    />
                  ) : null}
                  ...
                  <TaskType
                    ...
                    description={intl.formatMessage(
                      messages.tieudecongkhai || {
                        id: 'tieudemorong',
                        defaultMessage: 'tieudemorong',
                      },
                    )}
                    title={intl.formatMessage(
                      messages.nhommorong || { id: 'nhommorong', defaultMessage: 'nhommorong' },
                    )}
                  />
                </div>
              ) : (
            ...
            <div className="project-content">
              <div style={{ padding: 20 }}>
                <Grid item sm={12}>
                  <ButtonUI tabIndex={0}>
                    {intl.formatMessage(
                      messages.chitiet || { id: 'chitiet', defaultMessage: 'chitiet' },
                    )}
                  </ButtonUI>
                  ...
                  (!!taskOthers && taskOthers.edit === true) ? (
                    <ButtonUI tabIndex={1}>
                      {intl.formatMessage(
                        messages.coban || { id: 'coban', defaultMessage: 'coban' },
                      )}
                    </ButtonUI>
                  ) : null}
                  {(!!taskManagerRole &&
                    taskManagerRole.updateProgress === true &&
                    isTaskManager) ||
                  ...
                  (!!taskOthers && taskOthers.updateProgress === true) ? (
                    <ButtonUI tabIndex={2}>
                      {intl.formatMessage(
                        messages.tiendo || { id: 'tiendo', defaultMessage: 'tiendo' },
                      )}
                    </ButtonUI>
                  ) : null}
                  ...
                  <ButtonUI tabIndex={3}>
                    {intl.formatMessage(
                      messages.tailieu || { id: 'tailieu', defaultMessage: 'tailieu' },
                    )}
                  </ButtonUI>
                  ...
                  (!!taskOthers && taskOthers.convertTask === true) ? (
                    <ButtonUI tabIndex={4}>
                      {intl.formatMessage(
                        messages.chuyencongviec || {
                          id: 'chuyencongviec',
                          defaultMessage: 'chuyencongviec',
                        },
                      )}
                    </ButtonUI>
                  ) : null}
              ...
              {tabIndex === 0 ? (
                <Grid container>
                  <Grid item md={5}>
                    <Typography variant="h5">
                      {intl.formatMessage(
                        messages.thongtinchitiet || {
                          id: 'thongtinchitiet',
                          defaultMessage: 'thongtinchitiet',
                        },
                      )}
                    </Typography>
                    <TypographyDetail data={addProjects.name}>{names.name}: </TypographyDetail>
                    {addProjects.isProject && (
                      <TypographyDetail data={addProjects.code}>Code: </TypographyDetail>
                    )}
                    {addProjects.projectId && (
                      <TypographyDetail data={addProjects.projectName}>
                        Tên dự án:{' '}
                      </TypographyDetail>
                    )}
                    <TypographyDetail
                      data={addProjects.customer ? addProjects.customer.name : null}
                    >
                      {names.customer}:{' '}
                    </TypographyDetail>
                    <TypographyDetail data={toVietNamDate(addProjects.startDate)}>
                      {names.startDate}:{' '}
                    </TypographyDetail>
                    <TypographyDetail data={toVietNamDate(addProjects.endDate)}>
                      {names.endDate}:{' '}
                    </TypographyDetail>
                    <TypographyDetail data={taskStatusArr[addProjects.taskStatus * 1 - 1]}>
                      {names.taskStatus}:{' '}
                    </TypographyDetail>
                    <TypographyDetail
                      data={
                        <span style={{ color: priotyColor[addProjects.priority * 1 - 1] }}>
                          {taskPrioty[addProjects.priority * 1 - 1]}
                        </span>
                      }
                    >
                      {names.priority}:
                    ...
                    <TypographyDetail
                      data={
                        addProjects.finishDate !== null ? toVietNamDate(addProjects.finishDate) : ''
                      }
                    >
                      {names.finishDate}:{' '}
                    </TypographyDetail>
                    <TypographyDetail
                      data={addProjects.createdBy ? addProjects.createdBy.name : null}
                    >
                      {names.createdBy}:{' '}
                    </TypographyDetail>
                    <Typography variant="h5">Thông tin người tham gia</Typography>

                    <TypographyDetail data={this.mapPeople(addProjects.taskManager)}>
                      {names.taskManager}:{' '}
                    </TypographyDetail>
                    <TypographyDetail data={this.mapPeople(addProjects.viewable)}>
                      {names.viewable}:{' '}
                    </TypographyDetail>
                    <TypographyDetail data={this.mapPeople(addProjects.inCharge)}>
                      {names.inCharge}:{' '}
                    </TypographyDetail>
                    <TypographyDetail data={this.mapPeople(addProjects.support)}>
                      {names.support}:{' '}
                    </TypographyDetail>
                    <TypographyDetail data={this.mapPeople(addProjects.approved)}>
                      {names.approved}:{' '}
                    </TypographyDetail>
                    <TypographyDetail
                      data={<People planPeople={addProjects.joinPlan} people={addProjects.join} />}
                    >
                      {names.join}:{' '}
                    </TypographyDetail>
                    <Typography variant="h5">Mô tả chi tiết</Typography>
                    <div
                      dangerouslySetInnerHTML={{ __html: addProjects.desHtml }}
                  ...
                  <Grid className="progress-column" item md={3}>
                    ...
                    <ProgressBar
                      fillColor={fillColor}
                      textCenter={`${Math.floor(day)}d ${hours.toFixed()}h`}
              ...
              {tabIndex === 2 ? (
                <Grid container>
                  ...
                  <Grid item md={'auto'} style={{ marginRight: 10 }}>
                    <Button
                      variant="outlined"
                      color="primary"
                      onClick={this.handleOpenDialogProgress}
                    >
                      <Update
                        style={{
                          marginRight: 5,
                      ...
                      />
                      {intl.formatMessage(
                        messages.capnhat || { id: 'capnhat', defaultMessage: 'capnhat' },
                      )}
                    </Button>
                  </Grid>
                  <Grid item md={6}>
                    <Button variant="outlined" color="primary" onClick={this.handleCheckProgress}>
                      <CheckCircleOutline
                        style={{
                          marginRight: 5,
                          fontSize: '1.3rem',
                  ...
                  <Grid item md={12} style={{ display: 'flex' }}>
                    <Typography style={{ fontWeight: 'bold' }}>
                      {intl.formatMessage(
                        messages.tiendovapheduyet || {
                          id: 'tiendovapheduyet',
                          defaultMessage: 'tiendovapheduyet',
                        },
                      )}
                    </Typography>
                  </Grid>
                  <Grid item md={12}>
                    <div>
                      ...
                      >
                        {intl.formatMessage(
                          messages.lichsu || { id: 'lichsu', defaultMessage: 'lichsu' },
                        )}
                      </Buttons>
                      <Buttons
                        onClick={() => this.setState({ tab: 2 })}
                        color={tab === 2 ? 'gradient' : 'simple'}
                        size="sm"
                      >
                        {intl.formatMessage(
                          messages.phesuyet || { id: 'phesuyet', defaultMessage: 'phesuyet' },
                        )}
                      </Buttons>
                    </div>
                  </Grid>
                  ...
                  <Dialog
                    ...
                    title={intl.formatMessage(
                      messages.capnhattiendo || {
                        id: 'capnhattiendo',
                        defaultMessage: 'capnhattiendo',
                      },
                    )}
                      ...
                      <React.Fragment>
                        ...
                        >
                          {taskStatusArr.map((item, index) => (
                            <MenuItem
                              disabled={this.caculeDisable(index + 1)}
                              key={item}
                              value={index + 1}
                              style={{ display: item === 'Chậm tiến độ' ? 'none' : '' }}
                            >
                              {item}
                            </MenuItem>
                          ))}
                        ...
                        <TextField
                          ...
                          select
                          label={names.priority}
                          value={addProjects.selectPiority || addProjects.priority}
                          name="priority"
              ...
              {tabIndex === 3 ? (
                <Grid item md={12} spacing={4}>
                  <Typography style={{ fontWeight: 'bold', fontSize: 18 }}>
                    {intl.formatMessage(
                      messages.danhsachtailieu || {
                        id: 'danhsachtailieu',
                        defaultMessage: 'danhsachtailieu',
                      },
                    )}
                  </Typography>
                ...
                <Grid container>
                  <Grid item md={12}>
                    <div>
                      ...
                      >
                        {intl.formatMessage(
                          messages.nguoiphutrach || {
                            id: 'nguoiphutrach',
                            defaultMessage: 'nguoiphutrach',
                          },
                        )}
                      </Buttons>
                      <Buttons
                      ...
                      >
                        {intl.formatMessage(
                          messages.nguoithamgia || {
                            id: 'nguoithamgia',
                            defaultMessage: 'nguoithamgia',
                          },
                        )}
                      </Buttons>
                    </div>
              ...
              {tabIndex === 5 && isProject === true ? (
                <Grid container>
                  <div>
                    <Buttons
                      onClick={() => this.setState({ tabContract: 0 })}
                      color={tabContract === 0 ? 'gradient' : 'simple'}
                      size="sm"
                    >
                      {intl.formatMessage(
                        messages.hopdongnhacungcap || {
                          id: 'hopdongnhacungcap',
                          defaultMessage: 'hopdongnhacungcap',
                        },
                      )}
                    </Buttons>
                    <Buttons
                      onClick={() => this.setState({ tabContract: 1 })}
                      color={tabContract === 1 ? 'gradient' : 'simple'}
                      size="sm"
                    >
                      {intl.formatMessage(
                        messages.hopdongkhachhang || {
                          id: 'hopdongkhachhang',
                          defaultMessage: 'hopdongkhachhang',
                        },
                      )}
                    </Buttons>
                  </div>
                        ...
                        <TableRow>
                          ...
                          <TableCell
                            style={{
                              fontWeight: 'bold',
                              color:
                                totalArray(listRatio, 0, listRatio.length, 'ratio') === 100
                                  ? 'black'
                                  : 'red',
                            }}
                          >
                          ...
                          <TableCell />
                          <TableCell>
                            {totalArray(listRatio, 0, listRatio.length, 'costEstimate')}
                          </TableCell>
                        </TableRow>
                      </TableBody>
                    </Table>
          ...
          {(selectTask && id === 'add' && this.props.data.isProject) || tabIndex === 3 ? null : (
            <div
              style={{ display: 'flex', justifyContent: 'flex-end', padding: 10, marginRight: 40 }}
            >
