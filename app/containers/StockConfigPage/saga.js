...
import {
  ...
  getAllTrademarksSucsessAction,
  addTrademarkSuccessAction,
  updateTrademarkSuccessAction,
  updateTrademarkFailAction,
  addTrademarkFailAction,
  deleteTrademarksFailAction,
  deleteTrademarksSucsessAction,
} from './actions';
...
export function* getLocalStorage() {
  const value = yield localStorage.getItem('stockConfig');
  yield put(() => ({ type: 'GET_LOCALSTORAGE', payload: value }));
}
...
export function* fetchGetAllTrademarks() {
  try {
    ...
    if (data) {
      yield put(getAllTrademarksSucsessAction(data.data));
    } else {
      // yield put(getAllServicesSucsessAction({}));
    }
  } catch (err) {
...
export function* fetchAddTrademark(action) {
  const token = localStorage.getItem('token');

  try {
    const addService = yield call(request, SUPPLIER, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/x-www-form-urlencoded',
    ...
    if (addService) {
      let oldServices = yield select(makeSelectBody('trademarks'));
      oldServices = [addService.data, ...oldServices];

      yield put(addTrademarkSuccessAction(oldServices, 'Thêm thương hiệu thành công'));
    }
  } catch (err) {
    yield put(addTrademarkFailAction(err, 'Thêm thương hiệu thất bại'));
  }
}
export function* fetchUpdateTrademark(action) {
  const token = localStorage.getItem('token');

  try {
    const updateService = yield call(request, `${SUPPLIER}/${action.body._id}`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (updateService) {
      const oldServices = yield select(makeSelectBody('trademarks'));

      oldServices[oldServices.findIndex(d => d._id === updateService.data._id)] = updateService.data;

      yield put(updateTrademarkSuccessAction(oldServices, 'Sửa thương hiệu thành công'));
    }
  } catch (err) {
    yield put(updateTrademarkFailAction(err, 'Sửa thương hiệu thất bại'));
  }
}
export function* fetchDeleteTrademarks(action) {
  const token = localStorage.getItem('token');

  try {
    const deletedServices = yield call(request, `${SUPPLIER}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (deletedServices) {
      ...
      yield put(deleteTrademarksSucsessAction(newServices, 'Xóa thương hiệu thành công'));
    }
  } catch (err) {
    yield put(deleteTrademarksFailAction(err, 'Xóa thương hiệu thất bại'));
  }
}
...
export default function* stockConfigPageSaga() {
  ...
  // TRADEMARKS
  yield takeLatest(GET_ALL_TRADEMARKS, fetchGetAllTrademarks);
  yield takeLatest(DELETE_TRADEMARKS, fetchDeleteTrademarks);
  yield takeLatest(ADD_TRADEMARK, fetchAddTrademark);
  yield takeLatest(UPDATE_TRADEMARK, fetchUpdateTrademark);
}
