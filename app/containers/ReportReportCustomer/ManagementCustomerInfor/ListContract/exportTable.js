...
function ExportExcel(props) {
    ...
    const newData = data.map((item) => {
        const newCatalogContract = Array.isArray(listCatalogContract) && listCatalogContract.find(f => f.value === item.catalogContract);
        return {
            ...item,
            catalogContract: newCatalogContract
        }
    })
    ...
    return (
        <React.Fragment>
            {open ? (
                ...
                <table id="excel-table-list-contract" style={{ display: 'none' }}>
                    <thead>
                    ...
                    <tbody>
                        {newData.map((item, index) => (
                            <tr>
                                ...
                                <td style={{ border: '1px solid gray' }}>{item.customerId && item.customerIdName}</td>
                                <td style={{ border: '1px solid gray' }}>{item.name}</td>
                                <td style={{ border: '1px solid gray' }}>{item.catalogContract && item.catalogContract.title}</td>
                                <td style={{ border: '1px solid gray' }}>{item.contractSigningDate && moment(item.contractSigningDate).format("DD/MM/YYYY")}</td>
                                {/* <td style={{ border: '1px solid gray' }}>{item.startDay ?moment(item.startDay).format("DD/MM/YYYY"):null}</td> */}
