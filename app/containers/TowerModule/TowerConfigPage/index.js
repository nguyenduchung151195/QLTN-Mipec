...
export class ConfigTask extends React.Component {
  ...
  render() {
    ...
    const roleCode =
      ...
      this.props.dashboardPage.role.roles.find(item => item.codeModleFunction === 'Fee');
    const roleModule = roleCode && roleCode.methods;
    return (
      <div>
        <Paper>
          <Grid container md={12}>
            {roleModule && roleModule.find(item => item.name === 'GET').allow === true ? (
              <React.Fragment>
                <Grid item md={3}>
                  <VerticalTabs
                    value={tab}
  ...
  onCheck = (code, id) => {
    const roleCode =
      this.props.dashboardPage.role &&
      this.props.dashboardPage.role.roles &&
      this.props.dashboardPage.role.roles.find(item => item.codeModleFunction === 'TaskConfig');
    if (roleCode && roleCode.methods.find(item => item.name === 'PUT').allow === true) {
