...
import {
  ...
  getAssetById,
  resetNotiAct,
  createExportRecordAct,
  getOrderByIdAct,
  updateOrderExportAct,
} from './actions';
...
export class AddExportStockPage extends React.Component {
  ...
  state = {
    ...
    type: 1, // 0 đơn đặt hàng, 1 đặt hàng PO, 2 nhập hàng, 3 Chuyển kho,
    contractStage: '',
    products: [],
    paidList: [],
    allOrg: [],
  ...
  componentDidMount() {
    ...
    this.loadOptionsFirst(API_ORIGANIZATIONUSER, 'organizationUnitList');
    // if (!this.props.hocTable.dynamicForms) {
    //   this.props.history.push('/StockExport');
    // }
  }
  handleOnChangeOrg = e => {
    this.setState({ toOrg: e.target.value });
  };
  componentWillReceiveProps(props) {
    ...
    if (
      (this.state.type === 1 || this.state.type === 6 || (this.state.type === 2 && this.state.typeOfReturn === 1)) &&
      !this.state.isEditPage &&
      this.state.isQuickImport
    ) {
      let product = [];
    ...
    if (this.state.isEditPage && props.addExportStockPage !== this.props.addExportStockPage && props.addExportStockPage.order) {
      ...
      if (this.state.isEditPage && props.addExportStockPage.order !== this.props.addExportStockPage.order) {
        ...
        if (
          ...
          // if(type===3 || type ===7){
          //   this.props.onGetAssetById(serialize(params));
          // }
          if (type === 6 || type === 7) {
            this.props.onGetAssetById(serialize(params));
          } else {
        ...
        if (
          ...
          fetch(
            `${stockImport ? API_STOCK_IMPORT : salesQuotation ? API_SALE : ''}/${stockImport ? stockImport.stockImportId : salesQuotation ? salesQuotation.salesQuotationId : ''
            }`,
            {
              headers: {
        ...
        this.setState({
          ...
          isApproved,
          employee,
          toStock: type === 3 || type === 7 ? toStock.organizationUnitId : '',
  ...
  componentDidUpdate(props, prevState) {
    if (
      this.props.dashboardPage.profile &&
      this.props.dashboardPage.profile.workingOrganization &&
      this.props.dashboardPage.profile.workingOrganization._id
    ) {
      if (!this.state.isEditPage && this.state.type && prevState.type !== this.state.type) {
        this.state.toStock = this.props.dashboardPage.profile.workingOrganization._id;
      }
    }
  ...
  render() {
    ...
    const { classes, addExportStockPage, intl, dashboardPage } = this.props;
    const { allTemplates } = dashboardPage;
    const moduleTemplates = (allTemplates || []).filter(t => t.moduleCode === 'StockExport');

    const { discount, discountPercent, paymentAmount, name, toOrg, code, note, paidList } = this.state;
    ...
    return (
      <div>
        <AppBar className={classes.HearderappBarStockEx}>
          <Toolbar>
            <IconButton
            ...
            <Paper className={classes.paper}>
              <div style={{ display: 'flex' }}>
                ...
                >
                  ...
                  <MenuItem value={1}>Xuất hàng trực tiếp-theo sản phẩm</MenuItem>
                  <MenuItem value={6}>Xuất hàng trực tiếp-theo tài sản</MenuItem>
                  <MenuItem value={3}>Yêu cầu chuyển kho-theo sản phẩm</MenuItem>
                  <MenuItem value={7}>Yêu cầu chuyển kho-theo tài sản</MenuItem>
              ...
              <Paper className={classes.paper}>
                <Table>
                  <TableHead>
                    ...
                    {this.state.isEditPage || this.state.type === 6 || this.state.type === 7 ? null : <TableCell>Thao tác</TableCell>}
                  </TableHead>
                  <TableBody>
                  ...
                  {this.state.type !== 2 ? (
                    <React.Fragment>
                      ...
                      {/* <AsyncSelect
                        ...
                        loadOptions={(newValue, callback) => this.loadOptions(newValue, callback, API_ORIGANIZATIONUSER)}
                        loadingMessage={() => 'Đang tải ...'}
                        components={{ Option, SingleValue }}
                        // onBlur={() => field.onBlur({ value: field.value })}
                        onChange={this.handleOriganizationUser}
                        defaultOptions={this.state.organizationUnitList}
                        // inputValue={value}
                      ...
                      <TextValidator
                        ...
                        value={toOrg}
                        style={{ width: '100%', zIndex: 0 }}
                        onChange={this.handleChange('toOrg')}
                      />
                    </React.Fragment>
                  ) : (
                    <React.Fragment>
  ...
  handleSubmitForm = () => {
    ...
    if (type === 2 || type === 6) {
      ...
      //   return;
      // }
      // supplierRaw = supplier; //comment lại đợi thống nhất với khách hàng : comment lúc 8/12/2021
    }
    ...
    // };
    const x = allStock && allStock.find(item => item.id === this.props.dashboardPage.profile.workingOrganization._id);
    const fromStock = {
    ...
    const body = {
      ...
      dynamicFormsList: this.props.dashboardPage.allTemplates,
    };
    if (!this.state.isEditPage) {
      this.props.onCreate(body);
    } else {
      const { match, addExportStockPage } = this.props;
      const state = addExportStockPage.order && addExportStockPage.order.state;
      if (Number(state) === 4)
  ...
  handleOriganizationUser = value => {
    const origanizationUser = {
      origanizationUserId: value._id,
      name: value.name,
      //avatar: value.avatar,
    };
    this.setState({ organizationUnit: origanizationUser });
  };
