...
import { API_ASSET_MAINTENANCE, API_ORIGANIZATION, API_TASK_PROJECT, API_USERS } from '../../../../config/urlConfig';
...
function AssetMaintenanceTabPage(props) {
  ...
  const addNewAssetMaintenance = () => {
    history.push('/Maintenance/Asset/add');
  };

  const editAssetMaintenace = (item) => {
    history.push(`/Maintenance/Asset/${item._id}`);
  }
        ...
        <Grid item sm={12}>
          ...
          {currentTab === 2 && (
            <div>
              ...
              <ListPage exportExcel code="MaintenanceAsset" filter={filter} apiUrl={API_ASSET_MAINTENANCE} addFunction={addNewAssetMaintenance} onEdit={editAssetMaintenace} reload={reload} />
            </div>
          )}
          {currentTab === 3 && (
            <Planner code="MAINTENANCE" filterItem="planerStatus" apiUrl={API_TASK_PROJECT} category={5} addItem={handleCreateTask} reload={reload} />
          )}
