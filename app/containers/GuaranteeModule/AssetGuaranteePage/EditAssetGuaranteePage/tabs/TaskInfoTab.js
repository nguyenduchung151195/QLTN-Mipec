...
import { API_TASK_ASSET_GUARANTEE } from '../../../../../config/urlConfig';
...
function TaskInfoTab(props) {
  ...
  const { assetGuaranteePage, onCreateAssetGuaranteeTask, onDeleteAssetGuaranteeTask, classes, history } = props;

  const { assetGuarantee, createTaskSuccess, deleteTaskSuccess } = assetGuaranteePage;
  ...
  const handleSubmit = (task) => {
    onCreateAssetGuaranteeTask({ assetGuarantee, task });
  }
  ...
  const handleDeleteTask = (ids) => {
    onDeleteAssetGuaranteeTask({ assetGuarantee, ids })
  }
  ...
  return (
    <Grid container spacing={8}>
      <Grid item xs={12}>
        {assetGuarantee && assetGuarantee._id && (
          <GridList
            apiUrl={`${API_TASK_ASSET_GUARANTEE}/${assetGuarantee._id}`}
      ...
      <Grid item xs={12}>
        <SwipeableDrawer anchor="right" onClose={handleClose} open={open}>
          <EditTaskDialog task={task} onSubmit={handleSubmit}  type="GUARANTEE" onClose={handleClose} />
        </SwipeableDrawer>
      </Grid>
    </Grid>
  );
...
function mapDispatchToProps(dispatch) {
  return {
    onCreateAssetGuaranteeTask: (data) => dispatch(actions.createAssetGuaranteeTask(data)),
    onDeleteAssetGuaranteeTask: (data) => dispatch(actions.deleteAssetGuaranteeTask(data)),
  };
}
