...
function EditStockGuaranteePage(props) {

  
  const { stockGuaranteePage, classes, match, onGetStockGuarantee, onCreateStockGuarantee, onUpdateStockGuarantee, history, onCleanup } = props;
  ...
  const [stockGuarantee, setStockGuarantee] = useState(null);
  ...
  useEffect(() => {
    if (match.params && match.params.id !== 'add') {
      onGetStockGuarantee(match.params.id);
    }
    return () => {
      onCleanup();
    } 
  ...
  useEffect(() => {
    if (stockGuaranteePage.stockGuarantee) {
      const data = stockGuaranteePage.stockGuarantee;
      data.stock = data.stockId || '';
      data.contract = data.contractId || '';
      data.employee = data.employeeId || '';
      setStockGuarantee(stockGuaranteePage.stockGuarantee);
      //setTaskList(stockGuaranteePage.taskList);
    }
  }, [stockGuaranteePage.stockGuarantee])
  ...
  const handleSubmit = () => {
    if(!stockGuarantee.stock) {
      return;
    }
    const data = {
      ...stockGuarantee,
      stockId: stockGuarantee.stock && stockGuarantee.stock._id,
      contractId: stockGuarantee.contract && stockGuarantee.contract._id,
      startDate: stockGuarantee.startDate,
      expectedCompleteDate: stockGuarantee.expectedCompleteDate,
      employeeId: stockGuarantee.employee && stockGuarantee.employee._id
    }
    if (match.params && match.params.id === 'add') {
      onCreateStockGuarantee(data);
    } else {
      onUpdateStockGuarantee(data);
    }
  }
            ...
            <GridLT container spacing={16}>
              ...
              <Grid item xs>
                <SwipeableViews index={tabIndex}>
                  <TabContainer dir={1}>
                    <GeneralInfoTab onChange={setStockGuarantee} stockGuarantee={stockGuarantee || {}} />
                  </TabContainer>
                  {match.params.id !== 'add' && (
                    <TabContainer dir={2}>
...
EditStockGuaranteePage.propTypes = {
...
function mapDispatchToProps(dispatch) {
  return {
    onGetStockGuarantee: (data) => dispatch(actions.getStockGuarantee(data)),
    onCreateStockGuarantee: (data) => dispatch(actions.createStockGuarantee(data)),
    onUpdateStockGuarantee: (data) => dispatch(actions.updateStockGuarantee(data)),
