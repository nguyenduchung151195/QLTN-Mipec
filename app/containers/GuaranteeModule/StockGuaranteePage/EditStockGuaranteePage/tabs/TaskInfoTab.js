...
import { API_TASK_STOCK_GUARANTEE } from '../../../../../config/urlConfig';
...
function TaskInfoTab(props) {
  ...
  const { stockGuaranteePage, onCreateStockGuaranteeTask, onDeleteStockGuaranteeTask, classes, history } = props;

  const { stockGuarantee, createTaskSuccess, deleteTaskSuccess } = stockGuaranteePage;
  ...
  const handleSubmit = (task) => {
    onCreateStockGuaranteeTask({ stockGuarantee, task });
  }
  ...
  const handleDeleteTask = (ids) => {
    onDeleteStockGuaranteeTask({ stockGuarantee, ids })
  }

  return (
    <Grid container spacing={8}>
      <Grid item xs={12}>
        {stockGuarantee && stockGuarantee._id && (
          <GridList
            apiUrl={`${API_TASK_STOCK_GUARANTEE}/${stockGuarantee._id}`}
...
function mapDispatchToProps(dispatch) {
  return {
    onCreateStockGuaranteeTask: data => dispatch(actions.createStockGuaranteeTask(data)),
    onDeleteStockGuaranteeTask: data => dispatch(actions.deleteStockGuaranteeTask(data)),
  };
}
