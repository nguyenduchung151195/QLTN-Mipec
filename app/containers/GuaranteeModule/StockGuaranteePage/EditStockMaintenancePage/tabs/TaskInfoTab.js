...
import { API_TASK_STOCK_MAINTENANCE } from '../../../../../config/urlConfig';
...
function TaskInfoTab(props) {
  ...
  const { stockGuaranteePage, onCreateStockMaintenanceTask, onDeleteStockMaintenanceTask, classes, history } = props;

  const { stockMaintenance, createTaskSuccess, deleteTaskSuccess } = stockGuaranteePage;
  ...
  const handleSubmit = (task) => {
    onCreateStockMaintenanceTask({ stockMaintenance, task });
  }
  ...
  const handleDeleteTask = (ids) => {
    onDeleteStockMaintenanceTask({ stockMaintenance, ids })
  }
  ...
  return (
    <Grid container spacing={8}>
      <Grid item xs={12}>
        {stockMaintenance && stockMaintenance._id && (
          <GridList
            apiUrl={`${API_TASK_STOCK_MAINTENANCE}/${stockMaintenance._id}`}
      ...
      <Grid item xs={12}>
        <SwipeableDrawer anchor="right" onClose={handleClose} open={open}>
          <EditTaskDialog task={task} onSubmit={handleSubmit} type="MAINTENANCE" onClose={handleClose} />
        </SwipeableDrawer>
      </Grid>
    </Grid>
  );
...
function mapDispatchToProps(dispatch) {
  return {
    onCreateStockMaintenanceTask: (data) => dispatch(actions.createStockMaintenanceTask(data)),
    onDeleteStockMaintenanceTask: (data) => dispatch(actions.deleteStockMaintenanceTask(data)),
  };
}
