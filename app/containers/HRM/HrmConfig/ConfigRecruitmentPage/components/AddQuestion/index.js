...
function AddSubject(props) {
    ...
    return (
        <Grid container spacing={16}>
            ...
            <Grid item md={5}>
                <CustomInputBase label="Câu hỏi" name="name" value={localState.name} onChange={handleInputChange} />
            </Grid>
            ...
            <Grid item md={5}>
                <TextField select fullWidth name="type" value={localState.type} onChange={handleInputChange} label="Hình thức" InputLabelProps={{ shrink: true }}>
                    <MenuItem value={'text'}><ShortText />   Trả lời ngắn</MenuItem>
                    <MenuItem value={'checkbox'}><CheckBoxUI />   Trắc nghiệm</MenuItem>
                    <MenuItem value={'radio'}><RadioButtonChecked />   Đúng - Sai</MenuItem>
                </TextField>
            </Grid>
            <Grid item md={2}>
                <TextField select fullWidth name="obligatory" value={localState.obligatory} onChange={handleInputChange} label="Bắt buộc trả lời" InputLabelProps={{ shrink: true }}>
            ...
            </Grid>
            {
                localState.type === 'text' ? (
                    <>
                        <Grid item md={6}>
                            <CustomInputBase label="Văn bẳn trả lời ngắn" name="data" value={localState.data}  />
                        </Grid >
                    </>
                ) : null
            }
                                            ...
                                            return (
                                                <Grid item xs={12}>
                                                    <Grid container spacing={4}>
                                                        <Grid item xs={2}>
                                                            <CheckBox checked={localState.data[index].checked}
                                                                onChange={event => {
                                                        ...
                                                        <Grid item xs={8} className="px-2">
                                                            <CustomInputBase label="Câu trả lời" name="content" value={localState.data[index].content} onChange={event => {
                                                                const { data } = localState;
