...
import { API_DATA_SALARY, API_SALARY_FORMULA, API_ATTRIBUTE_FORMULA, API_ATTRIBUTE_LISTATTRIBUTE, API_HRM_PROJECT_BONUS } from '../../../../config/urlConfig';
import { serialize } from '../../../../helper';
import { changeSnackbar } from '../../../Dashboard/actions';
import {
  getAllDataSalarySuccess, getAllDataSalaryFailer, getAllDataSalary, updateDataSalarySuccess, updateDataSalaryFailer, getAllSalaryFormulaSuccess, getAllSalaryFormulaFailure, getAllSalaryFormula,
  getAttributeFormulaSuccess, getAttributeFormulaFailure, addSalaryFormulaSuccess, updateSalaryFormulaSuccess, deleteSalaryFormulaSuccess, addAttributeFormulaSuccess, addAttributeFormulaFailure, getListAttributeFormulaSuccess, getListAttributeFormulaFailure, addSalaryFormulaFailure, updateSalaryFormulaFailure, getProjectBonusFailure, getProjectBonusSuccess, addProjectBonusSuccess, addProjectBonusFailure, updateProjectBonusSuccess, updateProjectBonusFailure, deleteProjectBonusSuccess, deleteProjectBonusFailure, addSingleAttributeFormulaFailure, addSingleAttributeFormulaSuccess, updateSingleAttributeFormulaSuccess, updateSingleAttributeFormulaFailure
} from './actions';
import { ADD_SALARY_FORMULA, DELETE_SALARY_FORMULA, GET_ALL_DATA_SALARY, GET_ALL_SALARY_FORMULA, UPDATE_DATA_SALARY, UPDATE_SALARY_FORMULA, GET_ATTRIBUTE_FORMULA, ADD_ATTRIBUTE_FORMULA, UPDATE_ATTRIBUTE_FORMULA, DELETE_ATTRIBUTE_FORMULA, GET_LIST_ATTRIBUTE, GET_PROJECT_BONUS, ADD_PROJECT_BONUS, UPDATE_PROJECT_BONUS, DELETE_PROJECT_BONUS, ADD_SINGLE_ATTRIBUTE_FORMULA, UPDATE_SINGLE_ATTRIBUTE_FORMULA } from './constants';
...
export function* addAttributeFormulaSaga(action) {
  ...
  try {
    ...
    let newData = data.map(item => ({
      sourceAttribute: item.sourceAttribute.map(it => ({ name: it.name, code: it.code, _id: it.idOld, formula: it.formula ? it.formula : '', groupName: it.groupName || '' })),
      name: item.name,
...
export function* addSingleAttributeFormulaSaga(action) {
  ...
  try {
    ...
    const newData = {
      sourceAttribute: data.sourceAttribute.map(it => ({ name: it.name, code: it.code, _id: it.idOld, formula: it.formula ? it.formula : '', groupName: it.groupName || '' })),
      name: data.name,
