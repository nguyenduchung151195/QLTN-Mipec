...
function AddSocialInsurance(props) {
  const { socialInsurance, onSave, onClose, code } = props;
  ...
  useEffect(
    () => {
      if (socialInsurance && socialInsurance.originItem) {
        setLocalState({ ...socialInsurance.originItem });
      } else {
        setLocalState({});
      }
    },
  ...
  return (
    <>
      ...
      <Grid container spacing={8}>
        ...
        <Grid item xs={4}>
          <CustomDatePicker  label={name2Title.dateFounded || "Chọn ngày"} value={localState.dateFounded } name="dateFounded" onChange={e => handleInputChange(e,  true )} />
        </Grid>
        <Grid item xs={4}>
          <CustomDatePicker label={name2Title.dateNeed || "Chọn ngày"} value={localState.dateNeed} name="dateNeed" onChange={e => handleInputChange(e,  false )} />
        </Grid>
        <Grid item xs={4}>
...
AddSocialInsurance.propTypes = {
  // eslint-disable-next-line react/no-unused-prop-types
  // dispatch: PropTypes.func.isRequired,
};

export default memo(AddSocialInsurance);
