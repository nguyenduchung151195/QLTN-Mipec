...
import { GET_TIMEKEEPINGS, UPDATE_WAGES, DELETE_WAGES, UPDATE_TIMEKEEPING_CELL, GET_ALL_TIMEKEEP_TYPE } from './constants';
...
export function* fetchTimekeepings(action) {
  try {
    ...
    const response = yield call(request, `${API_TIMEKEEPING}/${tableId ? tableId : 'currentMonth'}?${serialize(rest)}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
