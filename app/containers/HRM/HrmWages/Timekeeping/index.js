...
function TimekeepingPage(props) {
  const { timekeepingPage, dashboardPage, tableId, onGetTimekeepingData, onSaveCellData, onGetAllTimekeepType, organizationUnitId, onClose } = props;
    ...
    <>
      <Paper>
        <Grid container spacing={16} direction="row" justify="flex-start" alignItems="flex-start" style={{ width: '100%' }}>
          {expansive
            ? <Grid item container style={{ width: `${widthColumn}px`, maxHeight: '620px', overflowY: 'scroll', }}>
              <Grid item container  >
          ...
          <Grid item style={{ width: `calc(100% - ${widthColumn}px)`, }}>
            <TimekeepTable onClose={onClose} timekeepTypes={timekeepTypes} data={timekeepingData} allSymbol={allSymbol} onSaveCellData={onSaveCellData} updateCellDataSuccess={updateCellDataSuccess} />
