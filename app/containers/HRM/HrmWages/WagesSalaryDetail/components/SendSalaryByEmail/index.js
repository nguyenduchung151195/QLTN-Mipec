...
function ModalSalaryCalculationDetails(props) {
  const { open, onClose, onSendMail, rows, templates, listTitle } = props;
  ...
  useEffect(
    () => {
      setLocalData({ ...localData, data: rows });
    },
    [rows],
  );
    ...
    <React.Fragment>
      <Dialog open={open} onClose={onClose}>
        ...
        <DialogContent style={{ width: 600 }}>
          <CustomInputBase type="text" label="Tiêu đề" name="title" value={localData.title} onChange={handleChange} />
