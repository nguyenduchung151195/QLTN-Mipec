...
function mapCodeEquipmentToEmployee(data, hrm2equipment) {
  ...
  if (isArray(hrm2equipment) && hrm2equipment.length) {
    isArray(data) && data.length && data.map((item, index) => {
...
function ImportHrmTimeKeeping(props) {
  const { classes, timekeepingEquipment, hrm2equipment, getTimekeepingToEquipment, importTimeKeeping } = props;
  ...
  function formatTimekeepingToData(timekeeping) {
    const data = timekeeping.map(([date, day, codeEmployee, name, department, position, ...rest]) => ({ date, day, codeEmployee, name, department, position, data: [...rest] }))
    ...
    */
    ...
    function formatDate(d) {
      if (Number(d)) {
        return moment().set('y', 1900).startOf('y').add(d - 2, 'd').format('MM/DD/YYYY');
      }
      return d;
    }

    const newDataFormat = newGroupByCode.map(item => ({ ...item, data: item.data.map(it => ({ ...it, date: formatDate(it.date), data: convertTimes(it.data) })) }))
        ...
        return (
          <div>
            <Grid container spacing={16} md={6}>
              <Grid item xs={4}>
                <CustomInputBase type="number" value={localData.month} onChange={handleChange} name="month" label="Tháng" />
              </Grid>
              <Grid item xs={4}>
                <CustomInputBase type="number" value={localData.year} onChange={handleChange} name="year" label="Năm" />
              </Grid>
              <Grid item xs={4}>
                <CustomInputBase select label="Chọn máy chấm công" name="equipment" onChange={handleChange} value={localData.equipment}>
                  <MenuItem value={0}>Chọn máy chấm công</MenuItem>
                  {isArray(timekeepingEquipment) && timekeepingEquipment.length && timekeepingEquipment.map(equipment => {
            ...
            >
              <Table>
                <TableBodyImportTimekeeping data={localData.data} month={localData.month} year={localData.year} handleChangeImport={handleChangeImport} />
              </Table>
            </Paper>
          </div >
        );
            ...
            <Step key={label}>
              ...
              <StepContent>
                ...
                <Button
                  ...
                  disabled={
                    ((!localData.file && activeStep === 0) ||
                      ((activeStep === 1 && !localData.equipment)))
                  }
                >
...
export default compose(memo, withStyles(styles))(ImportHrmTimeKeeping);