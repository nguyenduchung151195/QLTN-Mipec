...
function HumanResourcesPage(props) {
  ...
  const [columnExtensions] = useState([{ columnName: 'name', width: 200 }])
    ...
    <React.Fragment>
      ...
      <SwipeableDrawer anchor="right" onClose={handleCloseHumanResource} open={open} width={window.innerWidth - 260}>
        <div style={{ padding: 15 }}>
          <EditHumanResources selectHumanResources={selectHumanResources} humanResource={humanResource} roles={fields} onClose={handleCloseHumanResource} onSave={handleSave} />
        </div>
      </SwipeableDrawer>
    </React.Fragment>
  )
