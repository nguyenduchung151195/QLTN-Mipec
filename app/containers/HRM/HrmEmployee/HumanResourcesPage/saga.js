...
import {
  getHumanResourceFailure, getHumanResourceSuccess, updateHumanresourceFailure, updateHumanResourceSuccess,
  addHumanResourceSuccess, addHumanresourceFailure, getHumanResource
} from './actions';
...
export function* getHumanResourceSaga(action) {
  ...
  try {
    ...
    if (res && res.status === 1) {
      ...
      const fields = Array.isArray(humanResourceSource) ? humanResourceSource.map(field => ({ name: field.code, title: field.name, checked: true })):[]
