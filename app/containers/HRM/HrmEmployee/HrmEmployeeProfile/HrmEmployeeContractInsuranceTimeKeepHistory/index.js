...
import { API_HRM_SHIFT } from '../../../../../config/urlConfig';
...
function ContractInsuranceAndTimeKeep(props) {
  ...
  return (
    <Paper className={classes.paper}>
      ...
      {addPersonnelState.workingInformation && addPersonnelState.workingInformation.map((item, index) => (
        <Grid container md={12} spacing={16}>
          <Grid item xs={3}>
            <CustomInputBase
            ...
            >
              {localEquipment.length > 0 && localEquipment.map((item, index) => {
                return (
          ...
          <Grid item xs={1}>{addPersonnelState.workingInformation.length > 1 ? <Delete onClick={() => delColumn(item)} style={{ cursor: 'pointer', marginTop: 20 }} /> : null}</Grid>
        </Grid>
      ))}
      {localEquipment.length > localWorkingInformation.length && <Button onClick={addColumn} variant="outlined" color="primary">
        Thêm trường
      </Button>}
