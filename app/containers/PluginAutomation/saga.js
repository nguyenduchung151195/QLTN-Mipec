...
export function* addAutomation(action) {
  ...
  try {
    const addBo = yield call(request, `${API_AUTOMATION}`, {
      method: 'POST',
      headers: {
    ...
    console.log(addBo,action.automation,'action.automationaction.automation')
  } catch (err) {
...
export function* updateAutomation(action) {
  const token = localStorage.getItem('token');
  try {
    const updateAutomation = yield call(request, `${API_AUTOMATION}/${action.automationCode}`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
