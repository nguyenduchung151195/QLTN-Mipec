...
function FluctuationsPage(props) {
  const { timekeepingPage, fluctuationsPage, dashboardPage, tableId, onGetTimekeepingData, onSaveCellData, onGetAllTimekeepType, getIncreasesOrDecreases } = props;
      ...
      <Grid container spacing={16} direction="row" justify="flex-start" alignItems="flex-start" style={{ width: '100%' }}>
        {expansive
          ? <Grid item container style={{ width: `${widthColumn}px`, maxHeight: '620px', overflowY: 'auto' }}>
            ...
            <VerticalDepartmentTree addUser={false} addHrm={true} departments={allDepartment} onChange={handleSelectDepart} departmentId={organizationUnitId} />
          </Grid>
        ...
        <Grid item style={{ width: `calc(100% - ${widthColumn}px)`, }}>
          <Grid container spacing={16}>
            <Grid item xs={12}>
              <CustomChartWrapper height="450px" onRefresh={getIncreasesOrDecreases} isLoading={loadingChart}>
                <ColumnXYChart id="chartIncreasesOrDecreases" data={increasesOrDecreases} style={{ width: '100%', height: '100%' }}
                  title="Biểu đồ biến động tăng giảm" // tiêu đề biểu đồ
