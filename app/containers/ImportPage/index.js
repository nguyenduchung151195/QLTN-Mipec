...
export class ImportPage extends React.Component {
  ...
  componentWillMount() {
    ...
    if (currentViewConfig || this.props.columns) {
      ...
      // const typeImport = list.filter(item => item.typeImport);
      // const typeName = typeImport.map(item => `${item.name}`).join('-');
      ...
      { console.log(fieldIndexObj, 'cvbvcbcvbvc') }
      this.setState({
        fields,
  ...
  importData = () => {
    ...
    const identiferField = this.state.filterImport !== '' ? mappedData[0].code : '';
    const importSelect =
  ...
  customMapping(value, config, isDisplay) {
    console.log(value, config, isDisplay)
  ...
  handleFile(file /* :File */) {
    if (file.size < 1024 * 1024 * 20) {
      this.props.onChangeSnackbar({ status: true, message: 'Tải lên thành công', variant: 'success' })
      // alert('cập nhật thành công')
    } else {
      this.props.onChangeSnackbar({ status: true, message: 'Cảnh báo: Dung lượng vượt quá 20MB', variant: 'error' })
      // alert('Dung lượng vượt quá 20MB')
      return;
    }
    this.setState({ file });
  ...
  getExportFields = fields =>
    fields.filter(item => {
      ...
      if ( item.type && item.type.includes('ObjectId')) {
        return true;
      }
  ...
  exportFileFilter = () => {
    ...
    let data = this.getExportFields(typeImportData).map(item => ({
      ...
      order: item.orderImport ? item.orderImport : item.order,
      type: item.type,
      width: item.width,
              ...
              : item.name === 'type' && item.title === 'KIỂU CÔNG VĂN'
                ...
                : item.type === 'String'
                  ...
                  : item.type === 'Date'
                    ? `"${date}`
                    : item.type === 'Number'
                      ? item.name === "allocation.status" ? 'Nhập số : (-1): chưa thêm vào cấp phát ; 0: chưa cấp phát; 1: đã cấp phát; 2: thu hồi' : 'Nhập số'
                      : item.type === 'Boolean'
                        ? 'true: đúng, false: sai'
                        : item.title,
    }));
  ...
  handleChangeImport = event => {
    ...
    if (event.target.value === 0) {
      newFieldsImport = fields.filter(e => e.importTable === true);
    } else {
      const typeImport1 = this.state.filterImport !== '' ? fields.find(item => item.name === event.target.value) : '';
      newFieldsImport = typeImport1 ? typeImport1.typeImport : '';
    }
    // let newFieldsImport = typeImportData ? this.getExportFields(typeImportData) : null;
    this.getExportFields(newFieldsImport).forEach((item, index) => {
      fieldIndexObj[index] = item;
    });
    { console.log(fieldIndexObj, 'dsadasd') }
    this.setState({ [event.target.name]: event.target.value, fieldIndexObj: fieldIndexObj });
  ...
  getStepContent = step => {
    ...
    const viewConfig = JSON.parse(localStorage.getItem('viewConfig'));
    const dataField = viewConfig.find(i => i.code === codeField);
    const list = dataField.listDisplay.type.fields.type.columns;
    const typeImport = list.filter(item => item.typeImport);
    const typeName = typeImport.map(item => `${item.name}`).join('-');
  ...
  render() {
    ...
    const { openCountDialog, importCount, importedCount, flagDialog } = importPage;
