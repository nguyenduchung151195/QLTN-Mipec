...
import makeSelectStockPage, { makeSelectDashboardPage } from './selectors';
...
import { API_TAG_STOCK, API_SUPPLIERS, API_ADD_NEW_PRODUCT, API_LOWER_LIMIT_STOCK, API_UPPER_LIMIT_STOCK, API_BILLS } from '../../config/urlConfig';
...
export class StockPage extends React.Component {
      ...
      if (allProduct) {
        ...
        allOrg.forEach(unit => {
          ...
          if (newItem.type === 'stock' || newItem.type === 'salePoint') {
            this.state.allOrg.push(newItem);
          }
          if (unit.child) {
  ...
  customFunction = data => {
    ...
    try {
      newData = data.map((item, index) => ({
        ...
        'pricePolicy.costPrice': Number(item['pricePolicy.costPrice']).toLocaleString('es-AR', { maximumFractionDigits: 0 }),
        'pricePolicy.agentPrice': Number(item['pricePolicy.agentPrice']).toLocaleString('es-AR', { maximumFractionDigits: 0 }),
      }));
    } catch (err) {
      console.log(err, '');
    }
  ...
  addProduct = () => (
    <Add style={{ color: 'white' }} onClick={this.handleAddClick}>
      Thêm sản phẩm
    </Add>
  );
  ...
  render() {
    ...
    const { classes, stockPage, intl } = this.props;
    ...
    // }
    const filterTab = this.state.filter || {};
    return (
      <div>
        ...
        {this.state.openToEdit === false ? (
          <React.Fragment>
            ...
            <Paper>
              <TextField
                select
                label={intl.formatMessage({ id: 'app.containers.StockPage.loaisanpham' })}
                name="brand"
                variant="outlined"
              ...
              <Tabs value={this.state.tab} onChange={this.handleChange} indicatorColor="primary">
                ...
                <Tab
                  label={
                    <Badge color="primary" badgeContent={this.state.allLowerLimitProduct} max={9999}>
                      <Typography className={classes.padding}>
                ...
                <Tab
                  label={
                    <Badge color="primary" badgeContent={this.state.allUpperLimitProduct} max={9999}>
                      <Typography className={classes.padding}>
              ...
              {tab === 1 && (
                <ListPage
                  ...
                  extraColumns={extraColumns}
                  customFunction={this.customFunction}
                  filter={{ ...filterTab, limitStatus: 'below' }}
                />
              )}
              {tab === 2 && (
                <ListPage
                  ...
                  extraColumns={extraColumns}
                  customFunction={this.customFunction}
                  filter={{ ...filterTab, limitStatus: 'above' }}
                />
              )}
            </Paper>
            ...
            <DialogContent style={{ marginTop: 90, paddingTop: 10 }}>
              ...
              {this.state.stock === 0 ? (
                          ...
                          <Grid container>
                            <Grid md={6}>
                              <Typography style={{ textAlign: 'left', fontSize: '14px', marginLeft: '15px' }}>
                                Số lượng hiện tại:{' '}
                                {`${place.amount} ${place.additions < 0 ? '-' : '+'} ${place.additions < 0 ? -place.additions : place.additions
                ...
                <React.Fragment>
                  <Typography component="pre" style={{ fontSize: '20px', marginBottom: 10 }}>
                    Số lượng hiện tại:{' '}
                    {`${currentPoint.amount} ${currentPoint.additions < 0 ? '-' : '+'} ${currentPoint.additions < 0 ? -currentPoint.additions : currentPoint.additions
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {currentPoint.logs.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(item => {
                        ...
                        return (
                          <TableRow>
                            <TableCell>{`${item.createdAt.getDate() < 10 ? `0${item.createdAt.getDate()}` : `${item.createdAt.getDate()}`}-${item.createdAt.getMonth() + 1 < 10 ? `0${item.createdAt.getMonth() + 1}` : `${item.createdAt.getMonth() + 1}`
  ...
  handleEditClick = item => {
    ...
    history.push(`/Stock/edit/${item._id}`);
  };
