...
export class HistoryLog extends React.Component {
  ...
  handleChangerDate = (e, name) => {
    const { filter } = this.state;
    filter[name] = moment(e).format('YYYY-MM-DD');
        ...
        <GridMUI container className="p-3">
          <GridMUI item sm={3} className="px-2">
            <CustomDatePicker
              ...
              right={40}
              label="Từ ngày"
              value={filter.from === '' ? null : filter.from}
              margin="normal"
              variant="outlined"
              onChange={e => this.handleChangerDate(e, 'from')}
              InputLabelProps={{
                shrink: true,
              }}
              fullWidth
          ...
          <GridMUI item sm={3} className="px-2">
            <CustomDatePicker
              ...
              right={40}
              label="Đến ngày"
              value={filter.to === '' ? null : filter.to}
              margin="normal"
              variant="outlined"
              onChange={e => this.handleChangerDate(e, 'to')}
            />
          </GridMUI>
          <GridMUI item sm={3} className="px-2">
            <TextField
