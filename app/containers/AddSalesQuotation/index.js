...
export class AddSalesQuotation extends React.Component {
  ...
  render() {
    ...
    const payment = JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === 'S17').data;
          ...
          {dataGroup[i].map((item, index) => {
            return (
              <TableRow style={{ borderTop: index ? null : 'black solid 1px' }} key={item._id}>
                ...
                <TableCell>{item.pricePolicy ? this.converttoMoney(item.pricePolicy.costPrice * (1 - item.discount / 100) * item.amount) : 0}$</TableCell>
                <TableCell>
    ...
    return (
      <div>
        ...
        <KanbanStep handleStepper={this.handleChangeKanban} kanbanStatus={kanbanStatus} code="ST02" />
            ...
            <Paper className={classes.paper}>
              ...
              <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                {addSalesQuotation.order === 3 ? (
                  <AsyncAutocomplete value="" onChange={product => this.selectProduct(product)} url={API_STOCK} filters={['barcode']} />
                ) : null}
  ...
  changePaymentType = e => {
    const payments = JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === 'S17').data;
