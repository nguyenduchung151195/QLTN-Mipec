...
import { ItemBorder } from 'devextreme-react/funnel';
...
class TotalTask extends React.Component {
  ...
  componentDidMount() {
    ...
    const laneFail = [];
    currentCrmStatus &&
      Array.isArray(currentCrmStatus.data) &&
    ...
    const categoryTaskArr =
      JSON.parse(localStorage.getItem('taskStatus')) &&
      JSON.parse(localStorage.getItem('taskStatus')).find(item => item.code === 'TASKTYPE') &&
      JSON.parse(localStorage.getItem('taskStatus')).find(item => item.code === 'TASKTYPE').data;
  ...
  componentDidUpdate(preProps, preState) {
    ...
    // if (preState.search !== search || preState.filters !== filters) {
    // this.searchAdvance(this.state);
    // }
  ...
  onSaveDialog = () => {
    ...
    const bussines = roleTask.roles
      ? roleTask.roles.find(elm => elm.code === 'BUSSINES') && roleTask.roles.find(elm => elm.code === 'BUSSINES').data
      : [];
    const extra = roleTask.roles ? roleTask.roles.find(elm => elm.code === 'EXTRA') && roleTask.roles.find(elm => elm.code === 'EXTRA').data : [];
    const hideTask = roleTask.roles ? extra.find(elm => elm.name === 'hide') && extra.find(elm => elm.name === 'hide').data : false;
    const protectedTask = roleTask.roles ? extra.find(elm => elm.name === 'protected') && extra.find(elm => elm.name === 'protected').data : false;
    const publicTask = roleTask.roles ? extra.find(elm => elm.name === 'public') && extra.find(elm => elm.name === 'public').data : false;
    const openTask = roleTask.roles ? extra.find(elm => elm.name === 'open') && extra.find(elm => elm.name === 'open').data : false;

    const taskManager = roleTask.roles
      ? bussines.find(elm => elm.name === 'taskManager') && bussines.find(elm => elm.name === 'taskManager').data
      : false;
    const taskInCharge = roleTask.roles ? bussines.find(elm => elm.name === 'inCharge') && bussines.find(elm => elm.name === 'inCharge').data : false;
    const taskSupport = roleTask.roles ? bussines.find(elm => elm.name === 'support') && bussines.find(elm => elm.name === 'support').data : false;
    const taskViewable = roleTask.roles ? bussines.find(elm => elm.name === 'viewable') && bussines.find(elm => elm.name === 'viewable').data : false;
    const taskJoin = roleTask.roles ? bussines.find(elm => elm.name === 'join') && bussines.find(elm => elm.name === 'join').data : false;
    let time;
  ...
  render() {
    const { totalTask, profile, miniActive } = this.props;
    ...
    const extra =
      Array.isArray(this.props.dashboardPage.roleTask.roles) &&
      this.props.dashboardPage.roleTask.roles &&
      this.props.dashboardPage.roleTask.roles.find(elm => elm.code === 'EXTRA').data;
    const hideTask = Array.isArray(extra) && extra.find(elm => elm.name === 'hide') && extra.find(elm => elm.name === 'hide').data;
    const protectedTask = Array.isArray(extra) && extra.find(elm => elm.name === 'protected') && extra.find(elm => elm.name === 'protected').data;
    const publicTask = Array.isArray(extra) && extra.find(elm => elm.name === 'public') && extra.find(elm => elm.name === 'public').data;
    const openTask = Array.isArray(extra) && extra.find(elm => elm.name === 'open') && extra.find(elm => elm.name === 'open').data;
    ...
    const roleModuleAddProjectData =
      Array.isArray(roleModuleAddProject) &&
      roleModuleAddProject.find(elm => elm.name === 'add') &&
      roleModuleAddProject.find(elm => elm.name === 'add').data;
    ...
    return (
      <Paper>
        <GridLife container style={{ justifyContent: 'flex-end' }}>
          ...
          {tab === 4 ? (
            <GridLife item md={12}>
              <Automation status="taskStatus" path="/crm/Task" code="KANBAN" codeModule="Task" nameViewConfig={false} />
            </GridLife>
          ) : null}
          {tab === 1 ? (
            <GridLife item md={12}>
          ...
          {tab === 3 ? (
            <GridLife style={{ paddingTop: 20 }} item md={12}>
