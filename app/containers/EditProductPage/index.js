...
function createData(author, name, min, max, old, Latest, date) {
  return { author, name, min, max, old, Latest, date };
}
const getRowId = row => row.createdAt;
const columns = [
  { name: 'date', title: 'Ngày chỉnh sửa' },
  { name: 'min', title: 'Từ số' },
  { name: 'max', title: 'Đến số' },
  { name: 'old', title: 'Giá cũ' },
  { name: 'Latest', title: 'Giá mới' },
  { name: 'author', title: 'Người chỉnh sửa' },
...
export class editProductPage extends React.Component {
      ...
      <div>
        <CustomAppBar
          title={
            addStock === 'add'
              ? `${intl.formatMessage(
                  messages.themmoi || { id: 'themmoi', defaultMessage: 'Thêm mới kho' },
                )}`
              : `${intl.formatMessage(
                  messages.chinhsua || { id: 'chinhsua', defaultMessage: 'cập nhật sản phẩm' },
                )}`
          }
  ...
  handleSubmit = () => {
    ...
    if (Object.keys(productInfo).length !== 0) {
      ...
      let priceLadderList = pricePolicy.data.priceLadderList.map(el => {
        return {
          ...el,
          pricekWh: el.pricekWh ? el.pricekWh.toString().replaceAll('.', '') : 0,
        };
      });
      const pricePolicyRaw = {
        ...
        allPrice: pricePolicy.data.allPrice,
        priceLadderList: priceLadderList,
        agentPrice,
        taxs,
      };
      console.log(pricePolicyRaw);
      const sellingPoint = [];
      ...
      const otherInfo = {
        ...
        limitOrder:
          extendedInformation.data.orderLimit > 0 ? extendedInformation.data.orderLimit : 0,
        maximumLimit:
          extendedInformation.data.maximumLimit > 0 ? extendedInformation.data.maximumLimit : 0,
      ...
      // const { product } = this.props.editProductPage;
      // if (product) {
      //   let sellingPointArr = product.sellingPoint;

      //   sellingPoint.push(sellingPointArr[sellingPointArr.length - 1]);
      // }
      ...
      const body = {
        ...
        environment: productInfo.data.environment,
        tax: productInfo.data.tax,
        supplier,
      ...
      messages = Object.assign(
        productInfo.data.localMessages,
        pricePolicy.data.localMessages,
        extendedInformation.data.localMessages,
      );
...
function CustomGridList() {
  const [dateColumns] = React.useState(['date']);
  const [currencyColumns] = React.useState(['old', 'Latest']);
  React.useEffect(() => {
    fetchLog();
  }, []);
  return (
    <>
      <GridList rows={rows} columns={columns} getRowId={getRowId}>
