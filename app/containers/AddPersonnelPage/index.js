...
import { fetchAllUserAction, fetchListDepartment, fetchConfigAction, fetchUpdateConfigAction, resetNoti, mergeData } from './actions';
...
export class AddPersonnelPage extends React.Component {
  ...
  componentWillReceiveProps(props) {
    // if (props !== this.props) {
    //   const { addPersonnelPage } = props;
    //   if (addPersonnelPage.arrUser && addPersonnelPage.success) {
    //     this.state.data = addPersonnelPage.arrUser || [];
    //     this.state.pageDetail.totalCount = addPersonnelPage.count || 0;
    //     this.state.pageDetail.currentPage = Number(addPersonnelPage.skip || 0) || 0;
    //     this.state.pageDetail.pageSize = addPersonnelPage.limit || 0;
  ...
  mapFunctionProject = item => ({
    ...
    avatar: <Avatar src={item.avatar ? item.avatar : avatarA} />,
      ...
      <div>
        <Paper>
          ...
          {tab === 0 ? (
            ...
            //   </GridUI>
            //   <GridUI item md={9}>
            //     <ListPage code="hrm" status="hrmStatus" kanban="ST01" kanbanKey="_id" apiUrl={API_PERSONNEL} mapFunction={this.mapFunctionProject} filter={filter} />
            //   </GridUI>
            // </GridUI>

            <Grid container spacing={16} direction="row" justify="flex-start" alignItems="flex-start" style={{ width: '100%' }}>
              {expansive ? (
                <Grid item container style={{ width: `${widthColumn}px`, maxHeight: '620px', overflowY: 'auto' }}>
          ...
          {tab === 1 ? (
            <Kanban
              ...
              //callBack={() => this.props.mergeData({ openDrawer: true})} // sự kiện trả về kanban
          ...
          {tab === 2 ? (
            // <HrmTree />
            <HrmOrganization {...this.props.history} />
          ) : null}
          <SwipeableDrawer
            anchor="right"
