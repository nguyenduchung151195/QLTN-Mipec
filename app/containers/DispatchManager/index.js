...
export class DispatchManager extends React.Component {
  ...
  handleDelete = data => {
    this.props.onDeleteDispatch([data && data.data && data.data.event && data.data.event._id])
      ...
      >
        ...
        <div className="footer-kanban-item">
          <button type="button" className="footer-kanban-item-time">
            <Notifications style={{ fontSize: '1rem' }} /> {new Date(data.replyDeadline).toLocaleDateString('vi-VN', { month: 'long', day: 'numeric' })}
          </button>
