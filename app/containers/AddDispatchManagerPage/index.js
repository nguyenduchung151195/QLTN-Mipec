...
import { API_TASK_PROJECT, API_USERS, API_ROLE_GROUP, API_DISPATCH, API_MEETING, API_LOG, API_CUSTOMERS, API_ROLE_APP } from '../../config/urlConfig';
...
import { changeSnackbar, getProfileAct } from '../Dashboard/actions';
...
export class AddDispatchManagerPage extends React.Component {
  ...
  componentDidMount() {
    ...
    const getRoleApp = async () => {
      try {
        const res = await fetchData(`${API_ROLE_APP}/Documentary/${profile && profile._id}`);
        const { roles } = res;
        this.setState({
          access: roles && roles[1] && roles[1].data && roles[1].data[0] && roles[1].data[0].data && roles[1].data[0].data.access,
        });
      } catch (error) {
  ...
  componentWillReceiveProps(props) {
    if (this.props.addDispatchManagerPage !== props.addDispatchManagerPage) {
      props.addDispatchManagerPage.state === '4' ? (props.addDispatchManagerPage.kanbanStatus = '3') : props.addDispatchManagerPage.kanbanStatus;
  ...
  callbackTask = task => {
    ...
    this.props.mergeData({
      openDrawerTask: false,
      reloadTask: addDispatchManagerPage.reloadTask + 1,
    });
  };
  ...
  onSaveDucument = () => {
    ...
    if (this.props.match && this.props.match.url.includes('inComingDocument')) {
      if (new Date(addDispatchManagerPage.receiveTime) >= new Date(addDispatchManagerPage.replyDeadline)) {
        this.props.onChangeSnackbar({
          status: true,
          message: 'Hạn trả lời công văn lớn hơn thời điểm tiếp nhận',
          variant: 'warning',
        });
        return;
      }
    ...
    if (this.props.match && this.props.match.url.includes('outGoingDocument')) {
      if (
        (new Date(addDispatchManagerPage.toDate) >
        new Date(addDispatchManagerPage && addDispatchManagerPage.replyDispatch && addDispatchManagerPage.replyDispatch.replyDeadline)) && addDispatchManagerPage.replyDispatch
      ) {
        addDispatchManagerPage.kanbanStatus = '4';
      ...
      if (
        ...
        addDispatchManagerPage.kanbanStatus = '2';
      }
    }
    ...
    const data = {
      ...
      ['shippingUnit.name']: addDispatchManagerPage['shippingUnit.name'] || '',
      ['shippingUnit.shipperName']: addDispatchManagerPage['shippingUnit.shipperName'] || '',
      ['shippingUnit.time']: addDispatchManagerPage['shippingUnit.time'] || '',
      ['shippingUnit.phoneNumber']: addDispatchManagerPage['shippingUnit.phoneNumber'] || '',
      ['shippingUnit.unit']: addDispatchManagerPage['shippingUnit.unit'] || '',
      ...
      //   : [],
      customers: addDispatchManagerPage.customers,
      customerName: addDispatchManagerPage.customerName,
      mettingSchedule:
        localStorage.getItem('mettingScheduleId') && localStorage.getItem('mettingScheduleId') !== 'add'
    ...
    if (Object.keys(localMessages).length === 0) {
      ...
      this.props.onChangeSnackbar({
        status: true,
        message: 'Cần chọn đơn vị nhận',
        variant: 'warning',
      });
    } else {
      this.props.onChangeSnackbar({
        status: true,
        message: 'Trường màu đỏ không được để trống',
        variant: 'warning',
      });
    }
  };
  ...
  render() {
    ...
    console.log(476, addDispatchManagerPage);
    const { tab } = addDispatchManagerPage;
    const { names, checkRequired, checkShowForm, localMessages, height, types } = this.state;
    ...
    const type =
      this.props.documentType || this.props.location
        ? this.genMessageForRelationiField(this.props && this.props.location && this.props.location.pathname)
        : null;
    return (
      <div>
        <CustomAppBar
            ...
            type !== 'outGoingDocument'
              ? id === 'add'
                ? `${intl.formatMessage(
                    messages.chinhsua || {
                      id: 'chinhsua',
                      defaultMessage: 'Thêm mới công văn đến',
                    },
                  )}`
                : `${intl.formatMessage(
                    messages.chinhsua || {
                      id: 'chinhsua',
                      defaultMessage: 'cập nhật công văn đến',
                    },
                  )}`
              : id === 'add'
        ...
        <div>
          <KanbanStep
            ...
            kanbanStatus={addDispatchManagerPage.kanbanStatus}
            code={this.props.match && this.props.match.url.includes('outGoingDocument') ? 'ST14' : 'ST17'}
            disableClick={!this.state.access && this.props.match && this.props.match.url.includes('outGoingDocument') ? true : false}
          />
        </div>
        <div>
          <Grid container>
              ...
              <Grid container spacing={8}>
                <Grid item xs={4}>
                  ...
                  <CustomInputBase
                    ...
                    rows={4}
                    type={types.customerName}
                    onChange={e => this.props.mergeData({ customerName: e.target.value })}
                    value={addDispatchManagerPage.customerName}
                    error={localMessages && localMessages.customerName}
                    helperText={localMessages && localMessages.customerName}
                    required={checkRequired.customerName}
                    checkedShowForm={checkShowForm.customerName}
                  />
                  <CustomInputBase
                    id="outlined-basic"
                ...
                <Grid item xs={4}>
                  <Typography variant="subtitle2" gutterBottom>
                    {this.props.match && this.props.match.url.includes('outGoingDocument') ? 'Đơn vị phát hành công văn' : 'Đơn vị gửi'}
                  </Typography>
                ...
                {/* {type !== 'outGoingDocument' ? ( */}
                  <>
                    ...
                    <Grid container spacing={8}>
                      <Grid item md={4}>
                        <CustomInputBase
                          value={addDispatchManagerPage['shippingUnit.name']}
                          onChange={e => this.props.mergeData({ ['shippingUnit.name']: e.target.value })}
                          label={names['shippingUnit.name']}
                          error={localMessages && localMessages['shippingUnit.name']}
                          helperText={localMessages && localMessages['shippingUnit.name']}
                          required={checkRequired['shippingUnit.name']}
                          checkedShowForm={checkShowForm['shippingUnit.name']}
                        />
                      </Grid>
                      <Grid item md={4}>
                        <CustomInputBase
                          value={addDispatchManagerPage['shippingUnit.shipperName']}
                          onChange={e => this.props.mergeData({ ['shippingUnit.shipperName']: e.target.value })}
                          label={names['shippingUnit.shipperName']}
                          error={localMessages && localMessages['shippingUnit.shipperName']}
                          helperText={localMessages && localMessages['shippingUnit.shipperName']}
                          required={checkRequired['shippingUnit.shipperName']}
                          checkedShowForm={checkShowForm['shippingUnit.shipperName']}
                        />
                      </Grid>
                      <Grid item md={4}>
                        <CustomInputBase
                          value={addDispatchManagerPage['shippingUnit.time']}
                          onChange={e => this.props.mergeData({ ['shippingUnit.time']: e.target.value })}
                          label={names['shippingUnit.time']}
                          error={localMessages && localMessages['shippingUnit.time']}
                          helperText={localMessages && localMessages['shippingUnit.time']}
                          required={checkRequired['shippingUnit.time']}
                          checkedShowForm={checkShowForm['shippingUnit.time']}
                        />
                      </Grid>
                      <Grid item md={4}>
                        <CustomInputBase
                          value={addDispatchManagerPage['shippingUnit.phoneNumber']}
                          onChange={e => this.props.mergeData({ ['shippingUnit.phoneNumber']: e.target.value })}
                          label={names['shippingUnit.phoneNumber']}
                          error={localMessages && localMessages['shippingUnit.phoneNumber']}
                          helperText={localMessages && localMessages['shippingUnit.phoneNumber']}
                          required={checkRequired['shippingUnit.phoneNumber']}
                          checkedShowForm={checkShowForm['shippingUnit.phoneNumber']}
                        />
                      </Grid>
                      <Grid item md={4}>
                        <CustomInputBase
                          value={addDispatchManagerPage['shippingUnit.unit']}
                          onChange={e => this.props.mergeData({ ['shippingUnit.unit']: e.target.value })}
                          label={names['shippingUnit.unit']}
                          error={localMessages && localMessages['shippingUnit.unit']}
                          helperText={localMessages && localMessages['shippingUnit.unit']}
                          required={checkRequired['shippingUnit.unit']}
                          checkedShowForm={checkShowForm['shippingUnit.unit']}
                        />
                      </Grid>
                    </Grid>
                  </>
