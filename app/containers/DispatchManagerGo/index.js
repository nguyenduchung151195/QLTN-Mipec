...
import { mergeData, deleteDispatchGoAct } from './actions';
...
export class DispatchManagerGo extends React.Component {
  ...
  handleDelete = (data) => {
    ...
    this.props.onDeleteDispatchGo([data && data.data && data.data.event && data.data.event._id])
  };
    ...
    return (
      <div>
        <Grid container style={{ width: '100%' }}>
          ...
          <Grid item md={12}>
            {tab === 1 ? (
              <Kanban module="crmStatus" codeModule="outGoingDocument" reload={reload} code="ST17" apiUrl={API_DISPATCH_OUTGOING} itemComponent={this.ItemComponent} filter={{ type: '1' }} />
            ) : null}
            {tab === 0 ? (
              <Paper>
                <ListPage
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    onDeleteDispatchGo: ids => {
      dispatch(deleteDispatchGoAct(ids));
    },
  };
}
