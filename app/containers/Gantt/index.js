...
class Gantt extends React.Component {
  ...
  componentDidMount() {
    ...
    gantt.templates.task_text = function(start, end, task) {
      return `<span>${task.text} (${(task.progress * 100).toFixed(2)}% )</span>`;
    };
  ...
  loadData = async (search = '') => {
    ...
    const query = { limit: rowsPerPage, skip, filter };
      ...
      <div style={{ display: 'flex', width: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', marginLeft: 10, flex: 'auto' }}>
          ...
          <Menu id="simple-menu" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleClose}>
            <MenuItem onClick={props.exportFile} value={0}>
              PDF
            </MenuItem>
            <MenuItem onClick={props.exportFile} value={1}>
              PNG
            </MenuItem>
            <MenuItem onClick={props.exportFile} value={2}>
              Excel{' '}
            </MenuItem>
            <MenuItem onClick={props.exportFile} value={3}>
              iCal
            </MenuItem>
          </Menu>
        </div>
