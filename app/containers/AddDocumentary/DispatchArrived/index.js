...
function DispatchArrived(props) {
  const { onClose } = props;
  ...
  const customFunction = data => {
    return data.map(item => ({
      ...item,
      // fromUsers: item.fromUsers[0].name,
      // toDate: moment(item.toDate).format('YYYY'),
  ...
  const handleCloseExcel = () => {
    setOpenExcel(false);
    if (type === 'PDF') {
      let pdf = tableToPDF('excel-table-bos', 'reportDocumentaryIn', 'Báo cáo công văn đến');
      let win = window.open('', '', 'height=700,width=700');
      win.document.write(pdf);
      win.document.close(); // CLOSE THE CURRENT WINDOW.
      win.print();
    } else {
      exportTableToExcelXSLX('excel-table-bos', 'Báo cáo công văn đến');
    }
  };
  ...
  return (
    <React.Fragment>
      ...
      <CustomAppBar title={'Báo cáo công văn đến'} onGoBack={onClose} disableAdd />
      <div>
        <FilterReport
...
export default memo(DispatchArrived);
