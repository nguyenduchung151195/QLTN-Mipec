...
import { API_APPROVE_GROUPS, API_USERS_SEARCH, API_SEARCH_ORIGANIZATION } from '../../config/urlConfig';
...
export class HocTable extends React.Component {
      ...
      if (this.props.isClone) {
        this.props.onGetAPI({
          query: {
            ...
            sort: sort ? sort : this.state.sortValue,
          },
  ...
  componentWillReceiveProps(props) {
    if (props.data !== undefined) {
      ...
      if (functionAllow.includes('VIEWCONFIG')) {
        this.state.disableConfig = false;
      } else {
        this.state.disableConfig = true;
      }
      let currentViewConfigColumns = [];
      if (others) {
        currentViewConfigColumns = [...columns, ...others];
  ...
  handleChecked = index => {
    const newValue = this.state.viewConfig;
    newValue[index].checked = !newValue[index].checked;
    this.setState({ viewConfig: newValue });
  };
  handleCheckedRequireForm = e => {
    ...
    if (e.target.checked) {
      const newColumns = viewConfig.map(item => (item.name === curentColumns.name ? { ...item, checkedRequireForm: e.target.checked, checkedShowForm: e.target.checked } : item));
      ...
      <div>
        <Paper style={{ position: 'relative' }}>
          <Grid rows={newRows} columns={columns}>
            ...
            {/* TT - onColumnWidthsChange */}
            <TableColumnResizing
              columnWidths={this.state.columnsData}
            ...
            {this.props.disableSearch ? null : (
              <div style={{ marginLeft: 10, position: 'absolute', display: 'flex', alignItems: 'center' }}>
                <TextField
                  margin="dense"
                  ...
                  >
                    <div style={{ display: 'flex' }}>
                      <div>
                        ...
                        <MenuList style={{ height: 300, overflow: 'auto' }}>
                          {viewConfig
                            .filter(
                              item =>
          ...
          <DialogContent>
                ...
                item.type.includes('ObjectId') ? null : (
                  <GridMUI container alignItems="center">
                    <GridMUI item sm={10}>
                      <TextField
                        onChange={event => {
                          this.state.viewConfig[index].title = event.target.value;
                        }}
                        defaultValue={this.state.viewConfig[index].title}
                        autoFocus
                        margin="dense"
                        id="name"
                        label={item.title}
                    ...
                    <GridMUI item sm={2}>
                      <GridMUI container spacing={8}>
                        <Checkbox checked={this.state.viewConfig[index].checked} onChange={() => { this.handleChecked(index) }} value="checked" color="primary"
                        />
                        <Checkbox
                          inputProps={{
                            name: [item.name],
            ...
            {this.props.enableAddFieldTable === false ? (
              ''
                ...
                <Button variant="outlined" onClick={this.handleAddColumn} variant='outlined' color="primary">
                  Thêm trường
                </Button>
              )}
          </DialogContent>
        ...
        <Dialog maxWidth="md" fullWidth open={this.state.openPrint} onClose={this.handleClosePrintDialog} aria-labelledby="form-dialog-title1">
          ...
          <DialogActions>
            <Button onClick={this.handleClosePrintDialog} variant='outlined' color="secondary">
              Hủy
            </Button>
            <Button
              onClick={() => {
        ...
        <SwipeableDrawer anchor="right" onClose={() => this.setState({ openDrawer: false, id: 'add' })} open={this.state.openDrawer}  width={window.innerWidth - 260}>
        ...
        <DialogUI dialogAction={false} onClose={this.closeAssignTask} open={this.state.dialogAssign}>
          <AssignTask callbackAssign={this.callbackAssign} code={this.props.collectionCode} />
        </DialogUI>
      </div>
    );
  }
  ...
  handleAddColumnSave = () => {
    const { currentOrtherField, viewConfig } = this.state;
    const currentColumn = {
      checked: true,
      isFilter: false,
      isRequire: false,
