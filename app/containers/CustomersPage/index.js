...
const newColumns = (JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === CUSTOMER_TYPE_CODE) || { data: [] }).data;
const groupCustomer = (JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === 'S07') || { data: [] }).data;
const branchesCustomer = (JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === 'pckh') || { data: [] }).data;
const areasCustomer = (JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === 'S10') || { data: [] }).data;
const contactMethod = (JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === 'S06') || { data: [] }).data;
const typeProduct = (JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === 'S02') || { data: [] }).data;
const career = (JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === 'S12') || { data: [] }).data;
...
export class CustomersPage extends React.Component {
  ...
  mapFunctionCustomer = item => {
    ...
    const contactMethods =
      ...
      contactMethod.find(elm => elm.value === item.originItem.detailInfo.typeCustomer.contactWays[0]);
    const typeProducts =
      item.originItem.detailInfo.typeCustomer.productType &&
      typeProduct.find(elm => elm.value === item.originItem.detailInfo.typeCustomer.productType[0]);
    const areas2 =
      item.originItem.detailInfo.typeCustomer.areas && areasCustomer.find(elm => elm.value === item.originItem.detailInfo.typeCustomer.areas[0]);
    return {
      ...item,
      name: (
      ...
      birthDay:
        item.birthDay && moment(item.birthDay, 'YYYY-MM-DD').isValid()
          ? moment(item.birthDay, 'YYYY-MM-DD').format(localStorage.getItem('dateFomat'))
          : item.birthDay,
  ...
  callBack = (cmd, data) => {
    ...
    switch (cmd) {
      case 'create-bo':
        ...
        this.props.history.push(`/tower/Customer/add`);
        break;
        ...
        >
          ...
          <Grid container style={{ width: '100%' }}>
            <Grid item md={12}>
              ...
              {clientId === 'MIPEC' ? null : <Bt tab={1}>Dashboards</Bt>}
            ...
            <Grid item md={12}>
              {tab === 2 ? (
                <Automation
                  ...
                  path="/tower/Customer" // path để lấy viewconfig (hiển thị danh sách các trường bắt trigger)
                />
              ) : null}
              {tab === 1 ? (
                <CustomerDashboard
  ...
  handeCreateCampaign = ({ campaignName, title, template, enableSelectedAll }) => {
...
const customContent = [
  ...
  {
    title: 'Người quản lý',
    fieldName: 'managerEmployee.name',
    type: 'string',
  },
];
