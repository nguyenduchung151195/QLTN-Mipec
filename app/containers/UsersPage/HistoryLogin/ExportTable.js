  ...
  return (
    <React.Fragment>
      {open ? (
        ...
        <table id="excel-table-history-login" style={{ display: 'none' }}>
          <thead>
            ...
            {data &&
              data.map(mapFunction).map(item => (
                <tr>
                  ...
                  <td style={{ border: '1px solid gray' }}>{item.loginTime}</td>
                  <td style={{ border: '1px solid gray' }}>{item.logoutDate}</td>
                  <td style={{ border: '1px solid gray' }}>{item.username}</td>
                  <td style={{ border: '1px solid gray' }}>{item.organizationUnitId && item.organizationUnitId.name}</td>
                  <td style={{ border: '1px solid gray' }}>{item.employeeId && item.employeeId.name}</td>
                  {/* <td style={{ border: '1px solid gray' }}>{item.state}</td> */}
                </tr>
              ))}
          </tbody>
        </table>
