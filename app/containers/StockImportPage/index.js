...
import { Grid, TextField, MenuItem, Paper, Button, Tabs, Tab } from '@material-ui/core';
...
import { changeTabAct, getAllItemsAct, mergeStockImportPage } from './actions';
...
export class StockImportPage extends React.Component {
  ...
  mapFunction = item => {
    return {
      ...
      type:
        item.type === 2 ? 'Nhập hàng trực tiếp theo sản phẩm' :
          item.type === 3 ? 'Phê duyệt chuyển kho theo sản phẩm' :
            item.type === 6 ? 'Nhập hàng trực tiếp theo tài sản ' :
              item.type === 7 ? 'phê duyệt chuyển kho theo tài sản' : null
    };
  };
  ...
  handleChangeTab = (event, value) => {
    ...
    if (type === -1) {
      func = {
        filter: {
          ...
          state: { $in: Number(value) === 0 ? [4] : [0, 1, 2, 3] },
        },
        skip: 0,
        limit: 10,
      };
      ...
      this.props.mergeStockImportPage({ successUpdate: false })
      this.setState({ params: serialize(paramsX) }, () => this.props.mergeStockImportPage({ successUpdate: true }));
    } else {
      func = {
        filter: {
          type,
      ...
      this.props.mergeStockImportPage({ successUpdate: false })
      this.setState({ params: serialize(paramsY) }, () => this.props.mergeStockImportPage({ successUpdate: true }));
    ...
    this.props.mergeStockImportPage({ successUpdate: false })
    this.setState({ tab: value }, () => this.props.mergeStockImportPage({ successUpdate: true }));
  };
  render() {
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    mergeStockImportPage: val => {
      dispatch(mergeStockImportPage(val));
    },
  };
}
