...
import { getallOrgSuccess, getallOrgFailed, getAllCategorySuccess, getAllInventorySuccess, getAllTagsSuccess } from './actions';
...
export function* getallOrg() {
  try {
    ...
    if (data) {
      ...
      yield put(getallOrgSuccess(list));
    } else {
      yield put(getallOrgFailed({}));
    }
  } catch (err) {
    yield put(getallOrgFailed(err));
  }
}
...
export default function* addStockManagerSaga() {
  ...
  yield takeLatest(GET_ALL_STOCK, getallOrg);
