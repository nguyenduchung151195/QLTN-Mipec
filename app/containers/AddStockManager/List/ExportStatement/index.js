...
import { REPORT_STOCK_EXPORT } from '../../../../config/urlConfig';
...
const FIX_COLUMNS = [
  { name: 'name', title: 'Phiếu xuất', checked: true, width: 150 },
  { name: 'createdDate', title: 'Ngày tháng', checked: true, width: 150 },
  { name: 'note', title: 'Nội dung', checked: true, width: 150 },
  ...
  { name: 'importPrice', title: 'Đơn giá', checked: true, width: 150 },
  { name: 'totalPrice', title: 'Thành tiền', checked: true, width: 150 },
];
// const customColumns = FIX_COLUMNS.map(item => item.title);
// const customRows=
function ExportStatement(props) {
  const CODE = 'reportStockExportByProduct';
  ...
  //     ...item,
  //     // products: item.products, // originItem.products.totalPrice,
  ...
  const handleCloseExcel = () => {
    setOpenExcel(false);
    if (type === 'PDF') {
      let pdf = tableToPDF('excel-table-bos', 'report', 'Báo cáo xuất hàng');
      let win = window.open('', '', 'height=700,width=700');
      win.document.write(pdf);
      win.document.close(); // CLOSE THE CURRENT WINDOW.
      win.print();
    } else {
      exportTableToExcel('excel-table-bos', 'Báo cáo xuất hàng');
    }
  };
  ...
  return (
    <React.Fragment>
      <CustomAppBar title={'Báo cáo xuất hàng theo sản phẩm'} onGoBack={onClose} disableAdd />
      <Grid
        container
        spacing={16}
        direction="row"
        justify="flex-start"
        style={{ marginTop: 70, marginBottom: 8, paddingLeft: 16 }}
      >
        <Grid item xs={2}>
...
export default memo(ExportStatement);
