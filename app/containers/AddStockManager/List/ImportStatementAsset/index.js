...
function ImportStatement(props) {
  const { onClose } = props;
  const CODE = 'reportStockImportByAsset';
  ...
  const handleCloseExcel = () => {
    setOpenExcel(false);
    if (type === 'PDF') {
      let pdf = tableToPDF('excel-table-bos', 'report', 'Báo cáo nhập hàng theo tài sản');
      let win = window.open('', '', 'height=700,width=700');
      win.document.write(pdf);
      win.document.close(); // CLOSE THE CURRENT WINDOW.
      win.print();
    } else {
      exportTableToExcel('excel-table-bos', 'Báo cáo nhập hàng theo tài sản');
    }
  };
  return (
    <React.Fragment>
      <CustomAppBar title={'Báo cáo nhập hàng theo tài sản'} onGoBack={onClose} disableAdd />
      <Grid
        container
        spacing={16}
        direction="row"
