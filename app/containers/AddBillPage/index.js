...
import { getAllContractAct, createBillAct, resetNotiAct, getBillByIdAct, updateBillAct } from './actions';
...
export class AddBillPage extends React.Component {
  ...
  componentWillReceiveProps(props) {
    ...
    if (props !== this.props) {
      ...
      if (billSelected !== this.props.addBillPage.billSelected && this.state.isEditPage) {
        ...
        this.state.discount = Number(billSelected.discount) || 0;
  ...
  render() {
    ...
    const edittingTrading = JSON.parse(localStorage.getItem('edittingTrading'));
    const { classes, addBillPage, intl } = this.props;
    const {
      nameStaff,
      discount,
      discountPercent,
    ...
    return (
      <div>
        ...
        {/* <AppBar className={classes.HearderappBarAddBill}>
        <Toolbar>
          <IconButton
            // className={id !== 'add' ? '' : ''}
            className={classes.BTNAddBill}
            color="inherit"
            variant="contained"
            onClick={() => {
              const isEdittingTrading = JSON.parse(localStorage.getItem('edittingTrading'));
          ...
          <Typography variant="h6" color="inherit" style={{ flex: 1 }}>
            {addStock === 'add'
              ? `${intl && intl.formatMessage(messages.chinhsua || { id: 'chinhsua', defaultMessage: 'Thêm mới hóa đơn' })}`
              : `${intl && intl.formatMessage(messages.chinhsua || { id: 'chinhsua', defaultMessage: 'Cập nhật hóa đơn' })}`}
          </Typography>
          <Button
            variant="outlined"
            color="inherit"
       ...
       <CustomAppBar
          ...
          {/* <Grid md={4} item>
            <Paper className={classes.paper} style={{ height: '97px' }}>
              <Grid container item md={12} spacing={24} justify="center" alignContent="center" alignItems="center">
                ...
                {/* <div>
                  <Button aria-controls="simple-menu" aria-haspopup="true" onClick={this.handleClickHistory} color="primary" variant="outlined">
                    ...
                  </Button>
                  <Menu id="simple-menu" anchorEl={Boolean(history)} keepMounted open={Boolean(history)} onClose={this.handleClose('history')}>
                    <MenuItem onClick={this.handleClose('history')}>
                      <InsertDriveFile /> Lịch sử xuất kho
                    </MenuItem>
                    <MenuItem onClick={this.handleClose('history')}>
                ...
                {/* <Grid item md={5}>
                  <Button
                    ...
                    color="secondary"
                  >
                    <Cancel style={{ marginRight: '5px' }} /> Hủy hóa đơn
                  </Button>
                </Grid>
              </Grid>
            </Paper>
        ...
        <Grid item md={12} spacing={24} container>
          <Grid item md={8}>
            <Paper className={classes.paper}>
              <Table>
                ...
                <TableBody>
                  {arrProduct.map(item => {
                    const totalPrice =
                      item.costPrice * ((100 - item.discount) / 100) * item.amount * ((100 - (discountPercent !== '' ? discountPercent : 0)) / 100);
                    this.state.total += parseInt(totalPrice, 10);
                    ...
                    return (
                      <TableRow key={item._id}>
                        ...
                        <TableCell>{formatNumber((item.costPrice * (100 - item.discount)) / 100)}</TableCell>
                        <TableCell>{formatNumber(item.amount)}</TableCell>
                        <TableCell>{item.nameUnit}</TableCell>
                        <TableCell>{discountPercent}%</TableCell>
                        <TableCell>{formatNumber(parseInt(totalPrice, 10))}</TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>
            ...
            <Paper>
              <FileUpload name={this.state.name} id={id} code="Bill" />
            </Paper>
          </Grid>
          <Grid item md={4}>
            <Paper className={classes.paper}>
              <ValidatorForm style={{ width: '100%', display: 'inline' }} onSubmit={this.handleSubmitForm}>
                ...
                /> */}
                {this.checkShowForm('code') && <TextFieldCode
                  label={getLabelName('code', 'Bill')}
                  variant="outlined"
                  margin="normal"
                  name="code"
                ...
                {this.checkShowForm('name') && <CustomInputBase
                ...
                {this.checkShowForm('supplier') && <TextField
                  InputProps={{
                    readOnly: true,
                  }}
                ...
                {this.checkShowForm('sellEmployee') && <TextField
                  InputProps={{
                    readOnly: true,
                  }}
                ...
                {this.checkShowForm('discountPercent') && <TextValidator
                ...
                {this.checkShowForm('discount') && <TextValidator
                ...
                <hr />
                {this.checkShowForm('total') && <TextField
                ...
                {this.checkShowForm('remaining') && <TextValidator
                ...
                {this.checkShowForm('paidList') && <TextField
                ...
                {this.checkShowForm('deliveryDate') && <CustomDatePicker
                ...
                // error={deliveryDate === date && this.checkRequest('deliveryDate')}
                // helperText={(deliveryDate === date && this.checkRequest('deliveryDate')) ? localMessages.deliveryDate : null}
                />}

                {this.checkShowForm('orderDate') && <CustomDatePicker
                  ...
                  // error={orderDate === date && this.checkRequest('orderDate')}
                  // helperText={(orderDate === date && this.checkRequest('orderDate')) ? localMessages.orderDate : null}
                  />}
                ...
                {/* <TextField
                  ...
                  style={{ width: '100%' }}
                  onChange={this.handleChange('kanbanStatus')}
                ...
                {this.checkShowForm('note') && <TextField
