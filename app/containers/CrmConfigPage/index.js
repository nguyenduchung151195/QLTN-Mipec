...
export class CrmConfigPage extends React.Component {
  ...
  render() {
    ...
    console.log('this is crmCongfig', this.props.crmConfigPage);
    return (
      <div>
        <Helmet>
        ...
        >
          ...
          {this.state.type !== 'config' ? null : clientId === 'MIPEC' ? null : <Tab label="Tiền tệ" />}
          {this.state.type !== 'config' ? null : clientId === 'MIPEC' ? null : <Tab label="Địa điểm" />}
          {this.state.type !== 'config' ? null : clientId === 'MIPEC' ? null : <Tab lazbel="Thuế" />} */}
        </Tabs>
        {tabIndex === 0 && (
          <div className="my-3">
