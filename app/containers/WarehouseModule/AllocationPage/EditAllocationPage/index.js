...
function EditAllocationPage(props) {
  ...
  const handleSubmit = () => {
    // let id = props.id ? props.id : props.match.params.id !== 'add' ? 'update' : props.match.params.id;
    console.log(props.match.params.id, props.id);
    let id = props.id ? '' : props.match.params.id;
    let method = props.match.params.id ? props.match.params.id : 'update';
  ...
  return (
    <div>
      <CustomAppBar
        title={id === 'add' ? 'Thêm mới cấp phát' : 'cập nhật cấp phát'}
