...
class EditAssetTransfer extends React.PureComponent {
  ...
  handleChangeOldPersonManager = currentPersonManage => {
    this.setState({ currentPersonManage });
  };
  ...
  getData = () => {
    ...
    return {
      oldPersonManage: this.state.currentPersonManage ? this.state.currentPersonManage._id : '',
        ...
        <Grid item md={12} container spacing={24}>
          <Grid item md={12}>
            <Grid container spacing={8} alignItems="center">
              <Grid item xs>
                {/* <TextField label="Người quản lý" fullWidth value={asset && asset.personManage ? asset.personManage.name : ''} /> */}
                <AsyncAutocomplete
                  name="Chọn quản lý hiện tại..."
                  label="Người quản lý hiện tại"
                  onChange={value => this.handleChangeOldPersonManager(value)}
                  url={API_USERS}
                  value={this.state.currentPersonManage}
                />
              </Grid>
              <Grid item xs={1}>
                <SwapHorizIcon />
          ...
          <Grid item xs={12}>
            <SimpleListPage columns={columns} rows={asset ? asset.tranferPersonHistory : []} showAction={false} mapFunction={this.mapFunction} />
          </Grid>
        </Grid>
