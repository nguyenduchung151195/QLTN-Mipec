...
export class StockExportPage extends React.Component {
  ...
  mapFunction = item => {
    return {
      ...item,
      type: item.type === 1 ? 'Xuất hàng trực tiếp theo sản phẩm' :
        item.type === 2 ? 'Trả hàng' :
          item.type === 3 ? 'Yêu cầu chuyển kho theo sản phẩm' :
            item.type === 6 ? 'Xuất hàng trực tiếp theo tài sản' :
              item.type === 7 ? 'Yêu cầu chuyển kho theo tài sản'
                : null,
            ...
            <Grid item md={12}>
              <ListPage
                filter={{
                  ...
                  // type: { $in: Number(this.state.tab) === 0 ? [ 1, 2, 6] : [3, 7] }

                }}
                // defaultValue={{ state: Number(this.state.tab) === 0 ? 4 : 1 }}
                defaultValue={{ state: Number(this.state.tab) === 0 ? 4 : 0 }}
                titleHeader='Xuất kho'
